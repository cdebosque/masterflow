	...
	protected function _transco(EaiEvent $event) //beforeSend(EaiEvent $event)
	{
        ...
        if (true/*or (int)$data['grand_total'] > 0*/) {
            ...
            if (in_array($rootIncId, array('A812951', 'A813042', 'A813056'))
             or in_array($numero, array('1003339A', '1003430A', '1003492A', '1003491A', '1003738A'))
            ...
            $countryFacturation = strtoupper($data['shipping_country_id']);//code iso
            ...
            */
            $paymentMethods = array('ccsave'           => "CB"
                                  , 'checkmo'          => "Cheque"  //todo normalement, impossible
                                  , 'free'             => "unknow_free"
            ...
            if (round($data['base_tax_amount'], 2) == 0) {  //Si la TVA totale de la commane est ) 0
                $codeRegime = 'EI';
            ...
            foreach ($data['items'] as $index => $orderItem) {
                ...
                if (strtotime($data['created_at']) < strtotime('2013-01-01 00:00:00')  or !isset($trancoTauxStandardTVA[$countryFacturation])) {
                    //tant que tva que francaise ou pas de TVA... A changer le 1er janvier
                    ...
                    if ($tvaPercent > 7) {
                        ...
                        $codeTVA = 3;   // TVA reduite 7 pour les livres
                    } elseif ($tvaPercent > 0) {
                        $codeTVA = 2;   // TVA reduite 5.5 pour les jouets
                    } elseif ($tvaPercent == 0) {
                        if ($codeRegime == 'EI') {
                            $codeTVA = 1;
                        } else {
                            $codeTVA = 0;	// Pas de TVA. TODO 0 au 1er janvier
                        }
                    }
                } else {
                    if ($tvaPercent > $trancoTauxLivreTVA[$countryFacturation]) {
                        ...
                        $codeTVA = 3;   // TVA reduite 1 pour les livres
                    } elseif ($tvaPercent > 0) {
                ...
                */
                ...
                if ($tvaPercent == 0 && $codeTVA != 0) {
                    ...
                    var_dump("tvaPercent modifié $codeTVA $tvaPercent :", $tvaPercent);
                }
            ...
            dump($data['payment_method_id'], $method);
