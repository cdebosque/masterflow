<?php
class Observer extends EaiObserverProduct
{
    static $complexProduct=array();

    static $configurableProductsData=array();

    static $configurableAttribute= array(//'color_code',
                                         'ocp_size_code',
                                        );


    static $configurableSet= array(19=>'Chaussures',
                                   12=>'Couches',
                                   25=>'Textile',
                                   18=>'Vêtements',
                                   );


    public function onConnectOutFinish(EaiEvent $event)
	{
		$connector= $event->getObj();

		if ($connector->getClass()=='EaiConnectorSoap') {
    		self::setProductSetByKeysV2($connector);
    		self::setCategoriesByFullPathV2($connector);
		}
		elseif ($connector->getClass()=='EaiConnectorMage') {
    	    self::setProductSetByKeysMage($connector);
    	    self::setCategoriesByFullPathMage($connector);
    	    self::setWebsitesByIdsMage($connector);
		}
	}



	public function beforeSend(EaiEvent $event)
	{

		$rawData = $event->getObj()->getRawData();
		$sku = $rawData['sku'];
		unset($rawData['sku']);

		$type= $rawData['type'];
		unset($rawData['type']);

        if (!empty($rawData['ocp_size'])) {
            $sizeList= explode(',', $rawData['ocp_size']);
            $rawData['ocp_size']= $sizeList;
        } else {
            $rawData['ocp_size']= array('Defaut');
        }

        if ($rawData['attribute_set']=='Chaussures') {

            $rawData['ocp_shoes_size']= $rawData['ocp_size'];
            unset($rawData['ocp_size']);
        }

        if (!empty($rawData['ocp_matter'])) {
            $rawData['ocp_matter']= array($rawData['ocp_matter']);
        }


        if (empty($rawData['color'])) {
            $rawData['color']= 'Autres';
        } else {
            $colorList= explode(',', $rawData['color']);
            $rawData['color']= $colorList[0];
        }


        if (!isset($rawData['tax_class_id'])) {
            $rawData['tax_class_id']= 2;
        }

        if (!isset($rawData['website_ids'])) {

            if( !empty(self::$websitesByIds) ) {
                $rawData['website_ids']= array_keys(self::$websitesByIds);
            } else {
                $rawData['website_ids']= array(1);
            }
        }


        $from_attributeSetId= $rawData['attribute_set'];
        $attributeSetId= isset(self::$productSetByKeys[strtolower($rawData['attribute_set'])]) ? self::$productSetByKeys[strtolower($rawData['attribute_set'])]: 0;
        unset($rawData['attribute_set']);

        if (empty($attributeSetId)) {
            if(isset(self::$productSetByKeys['Default'])) {
                $attributeSetId= self::$productSetByKeys['Default'];
            } elseif(isset(self::$productSetByKeys['default'])) {
                $attributeSetId= self::$productSetByKeys['default'];
            }
            $event->getObj()->log('Set Default attribute_set for SKU: '.$sku, 'err');
        }

		if (is_numeric($type)) {
		    $keepCommonAttr= true;
		    $type= 'simple';
		    $rawData['visibility']= 1;

		    $simpleSpecificData= array();
		    foreach (self::$configurableAttribute as $attributeCode) {
                if (isset($rawData[$attributeCode])) {
                       $simpleSpecificData[]= array('code'=>$attributeCode, 'value'=>$rawData[$attributeCode]);
                }
		    }
		    self::$configurableProductsData[]= serialize(array('sku'=>$sku, 'attributes'=>$simpleSpecificData));

		} elseif ($type=='configurable' ) {


		    if (!isset(self::$configurableSet[$attributeSetId])) {
		        $event->getObj()->log('Not configurable product for SKU: '.$sku.' with attribute: '.$from_attributeSetId, 'err');
		        $event->getObj()->setRawData(false);
                        self::$configurableProductsData= array();
		        return true;
		    }

            if (!empty(self::$configurableProductsData)) {
    		    $rawData['additional_attributes']['multi_data'][]= array('key'=>'simple_attributes_data', 'value'=> self::$configurableProductsData);
            }

		    self::$configurableProductsData= array();
		}


		$from_categories= $rawData['categories'];

		if( is_int($from_categories) ) {
		    $rawData['categories']= array($from_categories);
		} else {
    		$rawData['categories']= self::getArrayIdsForCategoriesPath($rawData['categories'], 'Root Catalog/Catalogue dpam.com');
		}


		if( empty($rawData['categories'])) {
		    $rawData['categories']= array(859);
		    $event->getObj()->log('Set Dummy category for SKU: '.$sku, 'err');
		}


		foreach($rawData as $rawKey=>$rawValue)
		{
		    if (!in_array($rawKey,self::$allowProductV2Attributes, true)) {
		        if (is_array($rawValue)) {
		            $rawData['additional_attributes']['multi_data'][]= array('key'=>$rawKey, 'value'=> $rawValue);
		        } else {
		            $rawData['additional_attributes']['single_data'][]= array('key'=>$rawKey, 'value'=> $rawValue);
		        }
		        unset($rawData[$rawKey]);
		    }
		}

		if (!empty($rawData['categories']) and $attributeSetId) {

		    if(!empty($rawData['description'])) {
		        $rawData['description']= utf8_encode($rawData['description']);
		    } else {
		        $rawData['description']= $sku;
		    }

		    if(!empty($rawData['short_description'])) {
		        $rawData['short_description']= utf8_encode($rawData['short_description']);
		    } else {
		        $rawData['short_description']= $sku;
		    }

		    if ($event->getObj()->getClass()=='EaiConnectorMage') {
		        foreach($rawData['additional_attributes'] as $additionalKey=>$additionalValue) {
		            foreach($additionalValue as $dataKey=>$dataValue) {
		                $rawData['additional_attributes'][$additionalKey][$dataKey]= (object) $dataValue;
		            }
		            //$rawData['additional_attributes'][$additionalKey]= (object) $rawData['additional_attributes'][$additionalKey];
		        }
		        $rawData['additional_attributes']= (object) $rawData['additional_attributes'];

		        $rawData= (object) $rawData;
		    }

    		$custEaiData = array($type
    		                   , $attributeSetId
    		                   , $sku
    		                   , $rawData
    		);
		} else {
		    $event->getObj()->log('Cannot create product for SKU: '.$sku.' with attribute: "'.$from_attributeSetId.'" and category: "'.$from_categories.'"', 'err');
		    $custEaiData= false;
		}

        if( !empty($custEaiData) ) {
//             dump($custEaiData);
            //exit;
        }

		$event->getObj()->setRawData($custEaiData);
	}


	public function onFetchElementsInFinish(EaiEvent $event)
	{
		$sequencedElements = $event->getObj()->getElements();

		if (isset($sequencedElements[0]) && is_array($sequencedElements[0])) {
			$sequencedElement  = $sequencedElements[0];

			$newTmpElements = array();

			$commonAttributes = array('type'   => "simple");

			$countFA= 0;
			if (isset($sequencedElement['FA']) && is_array($sequencedElement['FA'])) {

			    //On ne prend que les produits valides
// 			    if( !self::isValidOcpSkuModel($sequencedElement['FA'][0][1]) ) {
//     			    $event->getObj()->log('skipping model:'.$sequencedElement['FA'][0][1]);
// 			        $event->getObj()->setElements(array());
// 			        return false;
// 			    }

			    $commonAttributes = array('type'   => "simple");

			    if(isset($sequencedElement['NCM']) and  is_array($sequencedElement['NCM'])) {
			        foreach ($sequencedElement['NCM'] as $ncmValue) {
			            if(!empty($ncmValue[4]) and !empty($ncmValue[3]) and $ncmValue[3]==10) {

			                switch ($ncmValue[3]) {
			                    case 10:
			                        $commonAttributes['ocp_collection']= $ncmValue[4];
			                    break;

			                    default:
			                    break;
			                }
			            }
			        }
			    }
    			if ( count($sequencedElement['FA'])>1 ) {
    			    $countFA= count($sequencedElement['FA']);
    			    $commonAttributes['type']= $countFA;
    			}

				foreach ($sequencedElement['FA'] as $index => $fa) {
					$newTmpElements[$index] = array_merge($commonAttributes, $fa);
				}
			} else {
			    return false;
			}

			//Tri des produits par couleur et positionnement du split
            usort($newTmpElements, 'self::sortByColorOn37');

            $colorAggregation= false;
            $newElements = array();
            $countTmpElements  =0;
            $countConfigProduct=0;
            foreach ($newTmpElements as $elemntKey=>$elementVal) {
                $simpleAdded= false;
                $countTmpElements++;
                $colorCode= $elementVal[37];

                if (empty($colorAggregation)) {
                    $colorAggregation= $colorCode;
                }

                //On ajoute le produit configurable
                if (($colorAggregation!==$colorCode or $countTmpElements==$countFA) and is_int($elementVal['type'])) {
                    $countConfigProduct++;
                    $elementValConfig= end($newElements);
                    $elementValConfig['type']  = 'configurable';
                    $elementValConfig['weight']= 1;
                    //Gestion du SKU
                    $elementValConfig[35]      = preg_replace('/\d+$/',$colorAggregation,$elementValConfig[35]);
                    //Gestion de EAN
                    $elementValConfig[36]      = $elementValConfig[36].str_pad($countConfigProduct, 5, '0', STR_PAD_LEFT);

                    //Le dernier element de la liste doit être ajouté avant son produit configurable
                    if ($countTmpElements==$countFA) {
                        $newElements[] = $elementVal;
                        $simpleAdded= true;
                    }
                    $newElements[] = $elementValConfig;
                    $colorAggregation= $colorCode;
                }

                //On n'est pas sur le dernier element donc on ajoute le produit simple
                if (!$simpleAdded) {
                    $newElements[] = $elementVal;
                }
            }

			$event->getObj()->setElements($newElements);
		}

	}//function


	public function sortByColorOn37($a, $b)
	{
	    return strcmp($a[37],$b[37]);
	}


	/**
	 * Encodage d'une chaine ou d'un tableau en UTF8
	 *
	 * @url http://naeh.net/convertir-un-array-en-utf8/
	 * @param mixed $param
	 */
	public function isoUTF8(&$value, $key=null)
	{
	    mb_detect_order('UTF-8, ISO-8859-15, ISO-8859-1, Windows-1252');
	    //parfois si le dernier caractère de la chaine est accentué, la conversion peut foirer,
	    //donc on force avec un caractère qui ne l'est pas.
	    //(astuce trouvée sur les commentaires de la doc sur php.net)
	    $currentCharset = mb_detect_encoding($value);

	    if ($currentCharset != 'UTF-8') {
    	    $value .= '_';
	        $value = mb_convert_encoding($value, 'UTF-8', $currentCharset);
	        $value = mb_substr($value, 0, mb_strlen($value)-1);
	    }

	    return $value;
	}


// 	public function onMap(EaiEvent $event)
// 	{
// 	    $sequencedElement = $event->getObj()->getElement();
// 	    dump($sequencedElement);
// // 	    exit;
// 	}

	public function isValidOcpSkuModel($model)
	{
	    $listValidModels= array(
	            'NEWPULL',
                'AVENTETIONE',
                'BAMBABYBOX',
                'BODYJLML',
                'CLAUDINECOUR',
                'CROICOURTBOD',
                'DROITCOURT',
                'BIKIFRESIA',
                'BOXCOLORS',
                'BOXIBIS',
                'BOXSEASTARS',
                'BORNBONNET',
                'BASIPANVELOU',
                'BASIFLATSOUP',
                'PORTECARTE',
                'PARCOMBELLE',
                'LAYTEECOURBA',
                'FREJEANDRESS',
                'MACQUEST',
                'LAYVELPANT',
                'VIPCARNET',
                'VIPTOUJO',
                'VIPTROUSSE',
                'VIPVOYAGE',
                'MOUK4FRFR',
                'P_TRICOGILAF',
                'P_DEBAGIRL',
                'P_ETECALGIRL',
                'VIPPRIM',
                'P_GILFINGIRL',
                'P_TEELONGIRL',
                'P_LAGTEECOU',
                'P_SUNCALAF',
                'P_SUNPANTLAF',
                'P_BOYPANT',
                'P_TEECOULAF',
                'P_PANTGIRL',
                'P_LAGSUNDUNG',
                'P_SUNJEANLAF',
                'P_BOYCHO',
                'P_CHOGIRL',
                'P_LAGCHO',
                'P_BOYDEBA',
                'P_BANDO',
                'P_BOYTEELON',
                'P_LOTSLIPS',
                'P_LEGMOUSLAF',
                'P_DEBALAF',
                'P_LAGDEBA',
                'P_LAGTEELON',
                'P_TEELONLAF',
                'P_BOYPOLOCOU',
                'P_BOYPOLOLON',
                'P_BOYTEECOU',
                'BODYBEMICROF',
                'BODYBEMICROG',
                'BODYBEUNIFI',
                'PYJGASO',
                'BODYBEUNIGA',
                'P_LEGCOTGIRL',
                'P_LEGCOTLAF',
                'BODYBEFINEFL',
                'P_CHOLONGIRL',
                'P_CHOLONLAF',
                'BODYBEFINEGL',
                'BODYBERAYGL',
                'P_LAGSOUP',
                'BODYBEUNIGAL',
                'P_CALMOLGIRL',
                'P_SOUPGIRL',
                'P_LAGWINJEAN',
                'P_WINPANTLAF',
                'P_CARDIGIRL',
                'P_BASEPULLO',
                'P_BOYSOUP',
                'P_SOUPLAF',
                'SACALANGER',
                'P_LEGMOUSGIR',
                'MACTECHNO',
                'ELLIOTCHEM',
                'REDCOUSSIN',
                'BEVELPANT',
                'STOKITFOREST',
                'BROSSEDPAM',
                'PYJANUISTAR',
                'TWEETYBLOUSE',
                'P_BOXER',
                'P_LAGSUNPANT',
                'PRALINPANT',
                'BRASSIETE',
                'AVENTPESBIG',
                'LEGENDSWEAT',
                'PYJFICOURT',
                'P_RYTHME',
                'ASTRUCJEUX',
                'ASTRUCS',
                'TRAINBOIS',
                'P_PIPEAU',
                'P_BOCHEVAL',
                'P_BELFILLE',
                'VENGEANCE09',
                'P_BEAUFILS',
                'P_MINIPOT',
                'P_FAGNE',
                'P_ILEBASICGA',
                'P_SONIE',
                'P_ENGLOIRE',
                'AVENTPESMALL',
                'P_ILEBASICFI',
                'BABYACTIVE',
                'REDCOMBIZIP2',
                'P_PANTJOGG',
                'P_LAGCHOLON',
                'P_SOBEAUTY',
                'P_SOPRETTY',
                'P_BOYROULPUL',
                'P_COUPVENT',
                'PYJGAMULTIRA',
                'C2SOBEAUTY',
                'P_SHERIE',
                'P_BETWEEN',
                'P_SOLONG',
                'P_FEDUBATO',
                'CLAUDINELONG',
                'CHOPYJBROD',
                'CHOPYJLAYFLE',
                'P_SOLUTION',
                'P_FAITPASCHO',
                'SHORTYBRASS',
                'BEOFFIDRESS',
                'P_SOQUEEN',
                'BRASSIEREMOD',
                'P_BOBEBEF',
                'P_BCBG',
                'DUFLORA',
                'MIROIRFDM10',
                'BEAGOUPILNEW',
                'LIVRERECANG',
                'ORACLEADI3G',
                'BELGUETRE3F',
                'BUGACAMHABI',
                'BUGACHANCEL',
                'BUGAOMBRELLE',
                'C3BELFILLE',
                'GATSBYBERMUG',
                'P_ELEGAMINE',
                'BOTHA3F',
                'BEABAVOIR',
                'UZESCHEM',
                'SHUSSPARKA',
                'LLIVRODEURS',
                'CHAISEXPLIAN',
                'PETERDUNGA',
                'HISSEOROBE',
                'DPAMASSIETTE',
                'P_BOBEBEG',
                'LMAXLILIJEU',
                'LQUELLEHEURE',
                'DORSNINA',
                'BECHECKBLOU',
                'SHIRECARDI',
                'BEASSIETTEVL',
                'C4DEBALAF',
                'RECRESHORT',
                'C4LAGDEBA',
                'C4TEECOULAF',
                'C4TEELONLAF',
                'SHIREBLOUSE',
                'THANKSCOMBGA',
                'VIPCOUSSIN',
                'BEKAPOLOBIS',
                'CHABEPOLOBIS',
                'BISIXCORDROB',
                'LPETITNAVIRE',
                'BELOLADOU',
                'BELOUIDOU',
                'BEPLUIPULL',
                'VAGUEPANT',
                'C3SOUPLAF',
                'BIOBODML',
                'DENISPANT',
                'SUNLAYBOY',
                'C4CHOGIRL',
                'DROLESET',
                'GANESHBLOU',
                'BEANOUKGIL',
                'BENEWSHIRT',
                'C4LAGCHO',
                'BISMYCARTEE',
                'POICHABODFI',
                'BETIPISKIRT',
                'BECORPANT',
                'BEBICOLTEE',
                'LAMBERPULL',
                'AKWAHADI1G',
                'POICHACOMBFI',
                'BENEWSOUP',
                'BERIBDEB',
                'BETINYDRESS',
                'P_SOCLOTLAF',
                'PETERCHEMBIS',
                'MARIUSS1G',
                'LUNETTAPRIL',
                'RIODUNGA',
                'OMISSY3F',
                'BOIFLOGIL',
                'LUMIBARBAPAP',
                'P_BATEAUVEL',
                'P_RAVA',
                'P_BATEAULACE',
                'GLADIE3F',
                'JOLI2F',
                'ELANA3F',
                'KAMILLOU1F',
                'KAMELA3F',
                'LDOUDOUCOPIN',
                'LPTIESCARGOT',
                'JLIVPUZZBOIS',
                'LJOLIEDECOUV',
                'LMEILLECHIEN',
                'BABYNOMADE2',
                'BOIFLOGASOCK',
                'SAM3G',
                'KARLINA3F',
                'SARALOU1F',
                'DIPOUPEES2F',
                'BIMOU2F',
                'IBIJOUX2F',
                'HELLOKY3F',
                'KOEURS3F',
                'ISAAK3G',
                'ESPIEGLOU3G',
                'HOURIA2F',
                'SHANDIDEBA',
                'COMECARDI',
                'ORGEATROBE',
                'SHANDIBLOUSE',
                'ZAKARI3G',
                'COMESOCK',
                'IPILLON2F',
                'POICHAFISOCK',
                'AVENTSUCETNW',
                'CAIRESOCK',
                'AFLOWERS3F',
                'C4ELEGAMINE',
                'POEMEPANT',
                'TONKYCHEM',
                'C5BOYJEAN',
                'C5PULLGIRL',
                'C5WILAFIPANT',
                'C5WINCALAF',
                'P_WILAFIPANT',
                'ZABEL3F',
                'C5BASEPULLO',
                'C5PANTJOGG',
                'BEPICTOTEE',
                'SAGISAROUEL',
                'C5BOYPOLOLON',
                'C5CALMOLGIRL',
                'TARAMANT',
                'TARAROBE',
                'BOXERBEBOY',
                'GRENIEPANTGA',
                'GRENIETEEGA',
                'CAIREPANTGA',
                'P_BODYFINEFL',
                'P_BODYFINEGL',
                'P_BODYMICROF',
                'VIDINDEB',
                'C5TEELONLAF',
                'PUZZLEBERMU',
                'P_BODYMICROG',
                'P_BODYRAYFI',
                'P_BODYRAYGA',
                'P_BODYRAYGL',
                'P_BODYUNIFI',
                'P_BODYUNIFIL',
                'BEDEBATRICO',
                'C5BOYTEELON',
                'C5TEELONGIRL',
                'P_BODYUNIGA',
                'P_BODYUNIGAL',
                'DOUNDOUILLET',
                'SAGITEE',
                'BELDOUNE',
                'BERAYURLEG',
                'BERAYURTEE',
                'DOTIEBLOU',
                'C5BOYSOUP',
                'LOTBESHORTY',
                'SAGITRICOT',
                'RECAROGROUPO',
                'ANTONGLOVES',
                'ANTONSCARF',
                'COMBIDOUNE',
                'ALANCHAPKA',
                'ALANMOUFLES',
                'BABECHEM',
                'SOFTOURSBIG',
                'SOFTLAPIN',
                'JANEPOINPULL',
                'JABBERSWEAT',
                'ORGEATSOUP',
                'PEINTURETAB',
                'JUDECHINPANT',
                'JUDEPOCKPANT',
                'ANTONVELPANT',
                'JANSLIMJEAN',
                'DOMIAM',
                'ANTODEUXENUN',
                'ANTODOUN',
                'ANTONBLOU',
                'AVADEUXENUN',
                'CRISTALCOMB',
                'P_BODYJLML',
                'AVADOUN',
                'AVAFANCYDOU',
                'JAMISBARNABE',
                'JLEPETITBAC',
                'LATABLE',
                'P_CROILONG',
                'C5SOCLOTLAF',
                'C5LAGSOUP',
                'SHANDIFISOCK',
                'BABEDUNGA',
                'P_CROICOURT',
                'JANFANCYJEAN',
                'JANWILDJEAN',
                'MIRZAGIL',
                'ORGEATSHORT',
                'BECANVIPANT',
                'COMBIFUN',
                'JUDJEANDROIT',
                'JUDLOOSJEAN',
                'LAPINPELUCHE',
                'MAKIROBE',
                'BODYMAISONS',
                'MARINJOGA',
                'PYJCANADA',
                'PYJPOUSSIN',
                'NUIDOUCE',
                'MAITRESSETAB',
                'DORSCIEL',
                'BODFIBECOTE',
                'BODYBOOK',
                'DOBOOK',
                'DOSHOES',
                'JANLOOSJEAN',
                'NUITCIEL',
                'JANEPOISOUP',
                'CROCKETPOLO',
                'FRIENDSWEAT',
                'AVABLOUSE',
                'AVAUNISHIRT',
                'JANEBLUSHIRT',
                'PYJFIRAYEBE',
                'RAMIPOLO',
                'RAMITEE',
                'MASKEPANT',
                'ASICAARON3G',
                'ASICORRIDO1F',
                'CITRONDRESS',
                'BEVELOUDUNGA',
                'BEVELOURPANT',
                'CELAPILOTEGA',
                'RINGJOGG',
                'WHERESHORT',
                'CONTBODFI',
                'CONTBODGA',
                'ASHLEYBLOU',
                'BEALLOBLOU',
                'NUITHARMOVA',
                'CINSETDE3G',
                'HARMOSARFI',
                'NOUSKASOUP',
                'CONTMANTFI',
                'CONTROBE',
                'HARMOMANTFI',
                'ILLUBODY',
                'HARMOBOFI',
                'P_IFOURREG',
                'HARMOTEEGA',
                'JOYGILJOGG',
                'JOYPANTJOGG',
                'JUDEPJPANT',
                'MARINPULLGA',
                'MARINROBTRI',
                'CONTBLOUFI',
                'CONTGILGA',
                'NOUSKAGIL',
                'ROUNDPANTJOG',
                'BEGOBISOUP',
                'BEHAPPYSOUP',
                'BELOUSOUP',
                'FRIENDSPANT',
                'AVAPOISPULL',
                'FLASHCOMBI',
                'NOEUPOLBON',
                'HARMOGILGAMO',
                'HARMOGIMOFI',
                'COLLEPULLGA',
                'TYPOBLOUSE',
                'JABBERCHEM',
                'QUESTDENIM',
                'ALVINSOUP',
                'BERUGBYPOLO',
                'CHOPYJSPACE',
                'P_SETTROUSSO',
                'BRISTOLSHIRT',
                'VELPYJVIP',
                'DOFIBEPOIS',
                'DORSHARMORAY',
                'TABVELDORS',
                'AVACARGOPANT',
                'HARMOBODGA',
                'DOCOW',
                'HARMOROBTRIC',
                'AVARIVPANT',
                'PEAKGLOVES',
                'FOXIEBON',
                'STEVELOSJEAN',
                'SUZYVELSLIM',
                'NIRVAROBE',
                'TARASOUP',
                'YINASOUP',
                'STEVEFANJEAN',
                'OLGAJACPULL',
                'OLGAPANT',
                'PINGUICAGO',
                'SUZYPANT',
                'DELFJEAN',
                'DELFSOUP',
                'FELTYPOLO',
                'MICROFIBRCOL',
                'NEWBORNCHOF',
                'JUDESANBELT',
                'FLEASKIRT',
                'BEVELJOGBAS',
                'KATCENVING3G',
                'WHEREPANT',
                'KARIBOUBON',
                'KARIBOUSCARF',
                'JUDEPOLO',
                'JUDMULTIPOLO',
                'BODYBISCOTO',
                'BODYLETTRE',
                'BODYSACADOS',
                'DOBEBOY',
                'P_SONIA',
                'ILLUGILET',
                'ILLUBONNET',
                'ILLUCOMBML',
                'CLASSICRIB0F',
                'LOTDEBCLASS',
                'NOEUPOLMOUF',
                'BEDENIMSAR',
                'BODYGACLASS',
                'BRISTOLPANT',
                'BEAUNOEUD3F',
                'DOFIRSTAR',
                'NINAN2F',
                'SARAAH3F',
                'APINGOUIN0G',
                'DESOURSONS1G',
                'DOURAYURE3F',
                'AVAMODGIL',
                'CIELBLOUSE',
                'DECARREAUX3G',
                'COLLETEEFI',
                'HOCKEY2G',
                'HOUPPER2G',
                'DELARAYURE1F',
                'DESROND1F',
                'DEURAYURES2G',
                'DIFUSEE1G',
                'KIKI1G',
                'ZARTY2G',
                'IBOUCLES3G',
                'PYJCOUETTE',
                'PYJFAON',
                'TICTOCROBE',
                'HOURTA3F',
                'RUYURE3G',
                'MINIBABY1F',
                'BERAYURPULL',
                'BRISTOLTEE',
                'MOUTONKGOULE',
                'BIOCOLNICER',
                'SHOCKBODGA',
                'SHOCKJOGGA',
                'STUDIO2G',
                'DIPAPILL1F',
                'DULION2G',
                'SHOCKPANTGA',
                'PYJAKING',
                'ZEUS3G',
                'ZIG3G',
                'DUPAPILL2F',
                'BODYNAIN',
                'DOMONDE',
                'ZINZIN3G',
                'MINOEUD1F',
                'HARMOBONGA',
                'BICOEURS3F',
                'BIOSOCKNICER',
                'AVAGRAFSOUP',
                'DURAYUVERT3G',
                'DUTAXI3G',
                'PYJBOUTIS',
                'PYJPOMETTE',
                'CHEMIQUEEN',
                'PYJTAXI',
                'SHORTYPOUSS',
                'ACAR0G',
                'ANTONMODBELT',
                'REFRAIN2F',
                'TARABLOU',
                'BABYVEL0G',
                'DOUILLETBON',
                'HARMOBONFI',
                'FREEZPULL',
                'OJUDE3G',
                'TARRA2F',
                'IBOILED2G',
                'IMOUTON2F',
                'SURPYJFUNK',
                'JUFAON2F',
                'VICHOO2G',
                'VICHOO3G',
                'BINESS3F',
                'KAROLINA3F',
                'DOBOIS',
                'DOFOLK',
                'DOTERRIER',
                'PYJFOREST',
                'BEMULCOSOUP',
                'BENEWPULL',
                'BICHEROBE',
                'BRODEVELROBE',
                'MASKEDUNGA',
                'WOODIPULL',
                'LOUISON2G',
                'ASICORRIDO2G',
                'ASICORRIDO2F',
                'SESCHAMPI2F',
                'SANA2F',
                'SECRET1F',
                'SELEFANT1G',
                'JACINTHE3F',
                'LORD3G',
                'BODYJAC',
                'BODYPTI',
                'DOPOTAGER',
                'DORSDOUX',
                'PYJAHEROES',
                'COLLEBODYGA',
                'KARENIBLOUFI',
                'KARENIDUNGAG',
                'KARENIPANTGA',
                'KIMONO3G',
                'COLLEGILGA',
                'SHOCKBONGA',
                'TREDOUCALE',
                'SHOCKPOLOFI',
                'ALPCOMBI',
                'ALPPARKA',
                'ALPSOUP',
                'ADDRAGON2G',
                'FAN2G',
                'INNSPANT',
                'INNSSOUP',
                'LULLU3F',
                'KENYA3F',
                'KYUDO3F',
                'NINA1F',
                'NOEMIE1F',
                'STEP3G',
                'JEROULE3G',
                'JIMONSTRE3G',
                'JOLICHAMPI1F',
                'STEVSLIMPANT',
                'STORMPANT',
                'WAKSHIRT',
                'NICO1G',
                'JOLIPLUMES2F',
                'KARYNIA3F',
                'KBOOTS3G',
                'KHAN3G',
                'KIKKOU3G',
                'KOKIN3F',
                'LCANARIPIPI',
                'LPETITEPOULE',
                'RENTAISE2F',
                'ISSNOW3F',
                'IMOUNTAIN3G',
                'GRENIECHEM',
                'TEBOUTONS3F',
                'JURAYLAYCOL',
                'BIFIG2F',
                'WALESSOUP',
                'DEGUIFEE',
                'BOGLAS2G',
                'WOCKYLAYCHO',
                'BOSOURIS0F',
                'EXTRA2G',
                'ANGEL2F',
                'ANNA2F',
                'GARDIANPOLO',
                'JEFFJEANS',
                'BLINKBELT',
                'AURONDS2F',
                'TWISTDRESS',
                'SEPERFOS1G',
                'SERG1G',
                'SESFLORS1F',
                'SEVELOURS1G',
                'OCTAVEPANT',
                'INDYBLOU',
                'MIABLOU',
                'MIAPANT',
                'TOUTSYGIL',
                'TOUTSYROBE',
                'OLGASOUP',
                'SUZYLICORSOU',
                'CIELMOUFI',
                'CIELSCARF',
                'KARENIROBE',
                'BIMBO2F',
                'ISARD1G',
                'ANTONMODPANT',
                'EDGARTEE',
                'ADICARS1G',
                'ADIVARIAL2G',
                'KART3G',
                'KARYNA3F',
                'HANSELFISOCK',
                'OLGABELT',
                'CAMEELAYCOL',
                'OLGAJACSKIRT',
                'SUZYDRESS',
                'SUZYHOODGIL',
                'BERCEHOUSSE',
                'STEVEPULL',
                'NOACHO',
                'CELACOMBIGA',
                'OLGASKIRT',
                'BEJACARDLEG',
                'COMEPANTFI',
                'DADPULL',
                'LOOPJEAN',
                'PATTYLEG',
                'OCTAGIL',
                'STEVBLENDTEE',
                'AVACAROSHORT',
                'AVADRESS',
                'AVAGLOVES',
                'AVAHAT',
                'GARDIANPULL',
                'INNSHAT',
                'BETTYDRESS',
                'MIRASOUP',
                'AVAMODPANT',
                'OLGAPULL',
                'BEMULTICOPUL',
                'EDGARBON',
                'EDGARCOMB',
                'HANDSWEAT',
                'PATTYJEAN',
                'INOEUD1F',
                'SUBWAY3G',
                'OCTAJEAN',
                'KARENIGILGA',
                'OCTAVEPULL',
                'AMORDRESS',
                'AMORPANTFI',
                'AMORSOUPFI',
                'HANSELPANTGA',
                'DADSOUP',
                'FLOCKJACPULL',
                'OLGACHEKBLOU',
                'OLGABLOUSE',
                'PONCHTEE',
                'RAINGILET',
                'BEWOOLCOMBI',
                'INDYPANT',
                'FUNSOUPU',
                'BENSEMBLE',
                'BESHORT',
                'NOASHIRT',
                'PYJIBISCUS',
                'AVAMODBLOUSE',
                'HANSELTEEGA',
                'SUZYDRESSTEE',
                'MOABPANT',
                'STEVRAITUNTI',
                'FORINPANT',
                'KHALOPANT',
                'POTTERDUNGA',
                'BEGANGTEE',
                'KHALOPULL',
                'FLOCKCOLOTEE',
                'FLOCKSHIRT',
                'OCTACHEEKTEE',
                'STEVEBANGTEE',
                'C6BANDO',
                'HANSESOUPFI',
                'ALPRAISOUP',
                'BEDEBGA',
                'BEPYJFIDOT',
                'BEPYJGARAY',
                'CHEMIFRUIT',
                'PYJFARM',
                'BECHEMIFIDOT',
                'DALIAGIL',
                'DALIARAYEPUL',
                'BOMABODYGA',
                'BOMAJEANSGA',
                'DALIASARJOG',
                'DALIASHIRT',
                'SUNCAMEL',
                'SUNLAYHADOCK',
                'BAINCOQ',
                'KARASKIRT',
                'LEEDGLOVES',
                'C6LAGSUNPANT',
                'C6LAGTEECOU',
                'NOUSKAMANT',
                'BRIEFRUIT',
                'DRESSMULTI',
                'PONCHSKIRT',
                'SETPODIUM',
                'C6BOYPOLOCOU',
                'C6JERGILAF',
                'C6TEECOULAF',
                'C6TEELONLAF',
                'AMORCHEMGA',
                'AMORSHIRTGA',
                'HANSELGILGA',
                'HANSELHATFI',
                'MARINEBOB',
                'OMBRELGIL',
                'OMBRELPANT',
                'OMBRELTEE',
                'AMOAMOSOCK',
                'NESSYGILET',
                'BEVELDOT',
                'BUGABEENIDA2',
                'BECADUNGA',
                'BODFLAMAND',
                'VIKSKIRT',
                'ALIAHJEAN',
                'DORSFIRST',
                'BAINLAYMARIE',
                'BAINRAYLAY',
                'CELASOUPULGA',
                'BOMACHEMGA',
                'BOMAGILETGA',
                'TEALAYBLOU',
                'TEALAYPULL',
                'TEAPARKA',
                'LOOPHAT',
                'HARMONIGONDI',
                'C6DEBAGIRL',
                'C6GILFINGIRL',
                'C6TEECOUGIRL',
                'MAYOLAYBOW',
                'PEIGNFLORA',
                'PEIGNRIVER',
                'SHORTYSECURE',
                'SLIPTEAM',
                'DOUGRIBGIL',
                'TEALAYJEAN',
                'DOFRUIT',
                'BIKIJUPDOTS',
                'PIXELDUNGA',
                'JADECOURPANT',
                'BOXGRAPH',
                'DALIAMARINTI',
                'FRESACOURJEA',
                'JOHNBASIPANT',
                'FABIOSLIMJEA',
                'JAJILLCHAPFI',
                'JAJILLROBEBL',
                'BECHESTRAYE',
                'BEVESTEJEANS',
                'BECAPANT',
                'JADEPULL',
                'JADESKIRT',
                'CALIJEANS',
                'DICKYDUNGA',
                'POTTERPOLO',
                'POTTERSWEAT',
                'COMBHIPOCAMP',
                'COMBIVAGUE',
                '3DPANTALON',
                'BODTRAVAUX',
                'BAINROSACES',
                'BIKIMULTI',
                'JEDESLANATUR',
                'LFIANCEPOURQ',
                'BODYZEBR',
                'BUGACOUSCONF',
                'CLICANOE',
                'COMBISHARK',
                'JADELEGG',
                'BODFRUIT',
                'SUNLAYSHARK',
                'BODGOMMETTE',
                'BODYFARM',
                'DOFARM',
                'DALIAFANCYJE',
                'C6SOCLOTLAF',
                'BEHISSEOTEE',
                'MAYOCTOPUS',
                'PYJICECREAM',
                'PARAJOHN',
                'MARINEROBE',
                'PLUMTYPANT',
                'PRINTPULL',
                'GHANAPANT',
                'NICYPANT',
                'C6LAGCHO',
                'JAJILLCALFI',
                'JAJILLTEEFI',
                'JADERAITEE',
                'EPURECHEMGA',
                'EPUREMICOMBI',
                'MARIMIPANTGA',
                'EPUREBODYGA',
                'EPUREFIBLOUS',
                'EPUREGAPANT',
                'FLAVIECOMBFI',
                'FLAVIEROB',
                'PARKABEBE',
                'CARAVELCOMBI',
                'MARIMICHEMGA',
                'SUNDOTS',
                'TIDYGIL',
                'MARIMIROB',
                'BEMARIPULL',
                'BESEVBLOU',
                'SACAPYJSOURI',
                'EPUREFIROB',
                'CORRALGIL',
                'GOCHOSHIRT',
                'NICYGIL',
                'SUNPAOLO',
                'GRACODUOSTAD',
                'PONCHDRESS',
                'C6BOYSOCLOT',
                'EDITOU6S3G',
                'KAMILLOU6S1F',
                'KAMOUFLAGE2G',
                'BROCCHO',
                'COLLACIROB',
                'GOLDBODYGA',
                'KISALOME2F',
                'KISALOME2G',
                'FRESAUNIDRES',
                'JADEBOLERO',
                'JADEBLOUSON',
                'MAYOLAYPOM',
                'FEPADRILLE3G',
                'BELRAYURES2F',
                'BODOTS2F',
                'BODRAFISO',
                'BOPOI1F',
                'WEDDPANT',
                'DOUGCHINEPUL',
                'ELOYBOB',
                'ELOYVESTE',
                'BABYBIOSOCK',
                'GRAFFTOPJOG',
                'PARKABIFAB',
                'SUSPEND6S2G',
                'BOMABANDO',
                'WEDDSHIRT',
                'BEFANCYPOLO',
                'SEFUCHO',
                'TROIDLAYCHO',
                'KETOU6S2F',
                'BEPRINTPANT',
                'YFLEUR3F',
                'AKWAH6S2G',
                'CARS2CF6S1G',
                'YFLOWERS3F',
                'BEVICHYBLOU',
                'BEZOEJEAN',
                'DUPIRATE1G',
                'LKTRAIN6S2G',
                'FOOTSET',
                'KARREAUX3G',
                'LOULAYBLOU',
                'LOULAYPANT',
                'LUNBLOU',
                'LUNPANT',
                'MARIMICOMBGA',
                'ESPIEGLE6S3F',
                'ESPIEGLE6S3G',
                'KATIANE6S2F',
                'BABYNOMADE',
                'ORAYURES3G',
                'OLUNETTES2G',
                'SUNNY6S3G',
                'CALIPOLO',
                'GRUNPANT',
                'COLACCIBERMG',
                'COLACCISHORG',
                'GATSBYCHEMGA',
                'FRESAPANT',
                'KINRALIVRE2',
                'DORISPANT',
                'BELFLEURS2F',
                'AUNI0G',
                'JAPAPILLE3F',
                'JAURBAN3G',
                'JODOJAPO1F',
                'SEPONPON3F',
                'SHORTYLINE3G',
                'JYCONFET3F',
                'SNICEFK6S3F',
                'VULCLEA6S3G',
                'BEPLAGESET',
                'MONOIDUNGA',
                'DORSGOLD',
                'NUITCOMEFI',
                'VIPDORSCOTE',
                'DORSEPURE',
                'ROFRUIT2F',
                'MAYOLAYCOEUR',
                'DOLIONGA',
                'SHORTYCEDRIC',
                'SUNGAPOOL',
                'KA373BN6S3G',
                'KE410PB6S2F',
                'GOCHOPANT',
                'DUTRAFIC1G',
                'OCOULEUR6S2F',
                'MEDUSEDEBA',
                'MEDUSEPANT',
                'BIFRAISIA3F',
                'BORSECURE3G',
                'BEZOEGIL',
                'LOCCABLOU',
                'LOCCAROBE',
                'MAXBELT',
                'NUITHANSEL',
                'DOBALADE',
                'MAXSHORT',
                'MILOJEANSHOR',
                'MIMOPICDRESS',
                '3DTEE',
                'BODYBEILLE',
                'BODYENVOL',
                'KE420LG6S2F',
                'KV373RW6S3G',
                'KV574RN6S1G',
                'BESALACOURT',
                'BEFRILDEB',
                'BERUFPOLO',
                'BEZOEDUNGA',
                'ENVUEBLOOM',
                'BIPONPON2F',
                'MAXLIONTEE',
                'SCOBAL3F',
                'ISPADRI3F',
                'ISUNFLOWER3F',
                'ZALOME1G',
                'IVICHY3F',
                'BOBENPLUS',
                'HAROLDCOMBI',
                'MONOISHIRT',
                'IBAIGNE6S2F',
                'CHIARATOP',
                'CLEASET',
                'BADMICOMBIFI',
                'KAISSAPANT',
                'KEYSKIRT',
                'SIRENDRESS',
                'SIRENPANT',
                'BOMARINGASOC',
                'MIKAPANT',
                'PENTYBOB',
                'YAKAPULL',
                'TZIGPANT',
                'DECHOCO3F',
                'DELAFLEU6S3F',
                'LILOOJUPE',
                'DESBRIDE6S2F',
                'DESLYS2F',
                'MIMOPOLO',
                'BEABIBOZMID',
                'ABYSCHO',
                'CHARMCOMBFI',
                'CHARMEBLOUFI',
                'COLACCICHEMF',
                'ENVUEFISOCK',
                'KALOU2F',
                'SAMY6S1G',
                'SANA6S2F',
                'SAPAPILE6S1F',
                'SOURIS0F',
                'LOOBABERMU',
                'MAXPOLO',
                'KE420BG6S2G',
                'CHARMEBODFI',
                'CHARMEBODYGA',
                'ZEBULE2F',
                'ZORION6S3G',
                'NODDY1F',
                'TAPISSOLPARC',
                'MESBRIDES2F',
                'MINTFLODRESS',
                'HOUNA2F',
                'EDIT1G',
                'MINOEUD6S1F',
                'MINOT0G',
                'MIROAD0G',
                'BUGABEECAPOT',
                'BUGACHANCEL2',
                'HELICO6S2G',
                'HELIO6S2G',
                'HOMFLEUR2F',
                'HONNOEUD1F',
                'HONOEUD1F',
                'HOPOIS2F',
                'ALDOLEMONTI',
                'BELFRANGES2F',
                'BOBIO6S2F',
                'MIMODEB',
                'ANDREA1F',
                'ILIANE3F',
                'KABOAT2G',
                'ILAND6S3G',
                'ISAAK6S3G',
                'CAMILLE6S1F',
                'BUGASACORGA',
                'MIMOSHORT',
                'BADMICHAPFI',
                'BADMIPANTGA',
                'AZURDRESS',
                'NANA1F',
                'NATURA1F',
                'NATURE1G',
                'LESUDSHORTFI',
                'GOSBYBERMU',
                'JANGOTEE',
                'LESUDBLOUFI',
                'MARTIA2F',
                'MINTMAILSKIR',
                'MINTOVERDEB',
                'ERIC3G',
                'JILPJCIRE',
                'JILPJPARKA',
                'LOUPIOMANT',
                'MATOUMANT',
                'CHICADEB',
                'C7TRICOGILAF',
                'KEYSTEE',
                'MAPBERMU',
                'ZINGASKIRT',
                'TETISTEPUP2',
                'BIBSTEPUP2',
                'NINNOU1G',
                'NUMBER1G',
                'HASSIANA2F',
                'C7BASEPULLO',
                'JAPTOPARKA',
                'JILTRENCH',
                'BEASACVIENNE',
                'C7SOUPLAF',
                'C7WILAFIPANT',
                'C7WINCALAF',
                'BALAP2F',
                'BHANEL1F',
                'BIFIG6S2F',
                'C7LAGTRICOGI',
                'JAPMAILBLOU',
                'STATESJOGG',
                'CHANCELMACLA',
                'DREAMTAB',
                'VIDEOGIL',
                'VIDEOJOGG',
                'ASIABASIPULL',
                'TOUTDOUFI',
                'ANNAMANT',
                'C7BOYGILET',
                'C7GILETGIRL',
                'CHEMIGIRLBE',
                'PARISBOOT',
                'SONGGILET',
                'C7TEELONGIRL',
                'P_GILETGIRL',
                'C7BOYTEELON',
                'LOUPIOPANT',
                'LOUPIOSOUP',
                'JAPHOODPULL',
                'C7LOTSLIPS',
                'MANUGAPRIM',
                'BEFIENSEMBLE',
                'LOTBEFICULOT',
                'LOTBEFITS',
                'BODPARTYFI',
                'BODYPOIDS',
                'BRIEFPARTY',
                'C7LAGTEELON',
                'C7LAGWINPANT',
                'C7TEELONLAF',
                'PYJBOYRAYBE',
                'C7CALMOLGIRL',
                'C7PULLGIRL',
                'PYJPARTYLAY',
                'JOEGIL',
                'C7BOYSOUP',
                'C7SOUPGIRL',
                'JOEJOGG',
                'BEBISOUP',
                'FIDOGILET',
                'HENDRIXSWEAT',
                'JILLEG',
                'JILSTARSOUP',
                'SANDYLEG',
                'PYJACLOUDY',
                'ANNAPARKA',
                'ASIAPARKA',
                'PYJAKITCHEN',
                'PYJFIRABBIT',
                'PYJGALAXY',
                'JAPRAIGIL',
                'LILITBLOU',
                'BECABGIL',
                'BELISDUNGA',
                'BESUSIEROBE',
                'DOGIRLBE',
                'BIONICERSOCK',
                'COLNICERBIO',
                'FAIRYTAB',
                'FEERIEDRESS',
                'SCHOOLTAB',
                'SEASGILET',
                'JAPSWEAT',
                'METALPANT',
                'BEPATCHSOUP',
                'JAPLOOSJEAN',
                'BECYCLETEE',
                'BEDEBAMOUSSE',
                'BETERYBLETEE',
                'DORSEASY',
                'ANNAKNOTDRES',
                'JILRAIPULL',
                'BLIZGILFI',
                'BLIZPANTGA',
                'BLIZPULLGA',
                'VELVIPPYJ',
                'LOUPIOMOUF',
                'ASIASLIMPANT',
                'JAPCARGOPANT',
                'LOYALGLOVES',
                'LOYALPOLO',
                'LOYALTEE',
                'PLANETROUSER',
                'BEJABERJEANS',
                'BERIBCHEM',
                'ECHARPEBB',
                'MEMORYDUNFI',
                'BETICTOTEE',
                'HENDRIXTEE',
                'PLANETEPULL',
                'PLANETSOUPUL',
                'MEMORCOEURFI',
                'ANNAPULL',
                'ANNARAISOUP',
                'MEMORYGILFI',
                'MEMORYGILGA',
                'MEMORYHATGA',
                'MEMORYSARFI',
                'MOINEAUBODFI',
                'BLIZJOGGA',
                'BLIZROBE',
                'ANNEROBE',
                'ANNETEE',
                'BELOLAJEAN',
                'BELOVETEE',
                'DOUCEPILOTFI',
                'DOUCEPILOTGA',
                'MATOUMOUF',
                'MATOUSCARF',
                'MEMORYBODGA',
                'MEMORYDUNGAG',
                'ANNAMODSHORT',
                'JILPOINPULL',
                'JILVELPANT',
                'CHEMIBALLET',
                'CHEMIGIRLRAY',
                'ANNAFOXTEE',
                'ANNAKNOTEE',
                'JILCOEURTEE',
                'JILFILLETEE',
                'JILHEARTEE',
                'COUKISHIRT',
                'FEERIEGIL',
                'JILRAICHO7',
                'MEMORYBODFI',
                'MEMORYTEEGA',
                'JILTIMETEE',
                'BETEDDYGIL',
                'FEERIESOUP',
                'KNOTSKIRT',
                'KNOTSLIMPANT',
                'PYJLUDLOVE',
                'PYJKROBOYONE',
                'ORYCOL',
                'ASIAFLAGTEE',
                'MOINEAUGILGA',
                'C7BOYCHOLON',
                'C7CHOGIRL',
                'C7CHOLONGIRL',
                'C7LAGCHOLON',
                'C7LEGCOTGIRL',
                'JAPTRAFFICTE',
                'ANNSKINJEAN',
                'C7LEGCOTLAF',
                'BELOLALEG',
                'BELOLATEE',
                'CHRYSMANT',
                'CLAPYBLOU',
                'CLAPYSHORT',
                'COPSGIL',
                'COPSTEE',
                'DORSMEMORYBI',
                'NUITMEMORYBI',
                'ICLAC2F',
                'METALSHIRT',
                'JAPFLEEGIL',
                'SEASRAITI',
                'SEASTEE',
                'JAPRAIPULL',
                'COOKICOL',
                'JAPRAISCARF',
                'JAPANHOODCHE',
                'JAPANVISHIRT',
                'JAPBOYJEAN',
                'ANNASHORT',
                'FOXICHAP',
                'BODYSWEET',
                'BODSOIREEGA',
                'DORSMOINEAU',
                'DOTUTU',
                'NUITBLIZ',
                'NUITMOINEAU',
                'JACAROBE',
                'LHASASOUP',
                'MARYGIL',
                'MARYJEAN',
                'TOURTEROBONF',
                'BALLPOLO',
                'BEBALLPOLO',
                'JUNEFIPRIM',
                'PYJASWEET',
                'MEMORYCALFI',
                'MEMORYTEEFI',
                'BERAYLAYCOL',
                'CLAPYLAYCOL',
                'FIDOLAYCHO',
                'GRETALAYCOL',
                'BELOLAGIL',
                'ANNAFLEEDRES',
                'ANNAKNOTPULL',
                'ANNAMILDRESS',
                'JILILITEE',
                'BAGGYPANT',
                'BAGGYTEE',
                'COUKIMITTEN',
                'KNOTPULL',
                'KNOTSHIRT',
                'ORYSOUP',
                'ORYTEE',
                'KNOTCOL',
                'LHASALAYCHO',
                'LHASALAYCOL',
                'MATOULAYCHO',
                'GRETASOUP',
                'GRETATEE',
                'KATYBLOU',
                'MARYBLOU',
                'NOEULEG',
                'NOEUROBE',
                'SOONTEE',
                'MATOULAYCOL',
                'TERRIBLAYCHO',
                'JAPFANCYJEAN',
                'JAPCARGOJEAN',
                'BALLTEE',
                'KAGOULETRICO',
                'MANIDUNGA',
                'METALGIL',
                'BRAGA07HT',
                'COLCOUKI',
                'JAPSLIMJEAN',
                'JILSLIMJEAN',
                'BEGASPETEE',
                'BETEDDYPANT',
                'MANITEE',
                'TOURTEROGIL',
                'ASIACAGOULE',
                'JAPREGJEAN',
                'JAPYOUNTEE',
                'JILRAILEG',
                'JILRAISOUP',
                'MOINEAUPANFI',
                'LOTSHORTPOIR',
                'FIDODUNGA',
                'BEECAPOTESPE',
                'CAMHABIEDSPE',
                'REDGROUPO2',
                'JILBOOTJEAN',
                'JILCOURJEAN',
                'PYJWILDPARK',
                'SCOOLPANT',
                'CARBONEDUNGA',
                'AARON6S1F',
                'ARIZONAHAT',
                'COUKIGIL',
                'COUKISOUP',
                'BEBABDUNGA',
                'BELOLAPULL',
                'MEMORFIBLOUS',
                'MEMORYJEANGA',
                'MOONYPANT',
                'ORYJEAN',
                'ASIAREGJEAN',
                'BEPILOUVESTE',
                'CHATROBE',
                'MANIGILET',
                'BEFUNPULL',
                'BETUBICBLOU',
                'MATOUJEAN',
                'JILJEANSKIRT',
                'JILRAITEE',
                'JILVELDRESS',
                'DICROCO2G',
                'DIVELOUR1G',
                'DIVOITURE1G',
                'JAPCHINOPANT',
                'JAPMULTIPOLO',
                'PANTINBOOT',
                'PANTINTEE',
                'SOONROBE',
                'BALLPANT',
                'DORSBLIZ',
                'JAPUNIPOLO',
                'NASAJEAN',
                'STOPPANT',
                'BEWOODPULL',
                'BESPACESWEAT',
                'BEKAISSAPANT',
                'GRETAPANT',
                'GRETAROBE',
                'MASKROBE',
                'JAPMAILPANT',
                'MEMORYDRESS',
                'JYMOUSSY2F',
                'ASIAMISTERTI',
                'BEANPANT',
                'MOONYDRESS',
                'SWEDMARINTEE',
                'OAKPOLO',
                'OAKSHIRT',
                'CARS7S1G',
                'HOUSLISTAR12',
                'SACCOUKI',
                'BRA07HT',
                'LKTRAINER71G',
                'MICROFICOL',
                'LKTRAINER71F',
                'LKTRAINER72F',
                'VARIALMID72G',
                'DELARAY7S1F',
                'DEPOIS2F',
                'DIBERTY3F',
                'DIRAISE2F',
                'DITOILE2F',
                'CHOBOYBEBIS',
                'CHOGALAXY',
                'CHOMULTIPOIS',
                'CHORABIT',
                'BODRAFY',
                'DUCOEUR7S3F',
                'DAYURE7S1G',
                'DAYURE7S3G',
                'ANNABLOUSE',
                'ABEE0F',
                'ANNAMODBLOUS',
                'ANNAMODGIL',
                'JYGALAXY3G',
                'DEREVE1F',
                'DIBERTY1F',
                'AUNI7S0G',
                'ANNAKNOTCHO',
                'ASIABELT',
                'BINECK3F',
                'BRIEFBALLET',
                'ASIASTORMTEE',
                'ASIAMARSTEE',
                'BIRAYURE3G',
                'BOYRAY1G',
                'BEYUREGIL',
                'SWEDRIBJEAN',
                'ANNASTARLEG',
                'JILMOONPULL',
                'COSMOLAYCHO',
                'BETTYBASH73F',
                'BREAKPOINT2G',
                'SOLIDGROUN3F',
                'SOLIDGROUN3G',
                'AMBRESKIRT',
                'AMBRETEE',
                'AMBREBLOUSE',
                'TRESSYGIL',
                'BODHAPPYFLAK',
                'ORYGIL',
                'ETEBLOUFI',
                'ETEPANTFI',
                'BICLOUDY3F',
                'BIGABOUH3G',
                'DOCITY',
                'NUITINDIEN',
                'RESERGE2G',
                'RIFLOWER7S2F',
                'CHOUCAPANT',
                'BEABIBPOIGNE',
                'STOPSWEAT',
                'ASIAMODPANT',
                'DRAGONCFI71G',
                'CARBONECHEM',
                'ASIARAIPOLO',
                'BELOLABLOU',
                'BOWVELROBE',
                'LASCOPANT',
                'CHOUCATEE',
                'CITADIDRESS',
                'CITADIGIL',
                'LHASABLOU',
                'LHASAPANT',
                'LHASAROBE',
                'CITADITEE',
                'CLUZADRESS',
                'CLUZASKIRT',
                'CLUZASOUP',
                'CLUZATEE',
                'FLODOTSDRES',
                'KAREEN3F',
                'TIDOUMIGIL',
                'OQUEBEC2G',
                'ICOSSAIS1G',
                'IGALIE1F',
                'ASIASWEAT',
                'ETEDUNGAGA',
                'TIYACKBLOUFI',
                'TIYACKGILFI',
                'TIYACKPANTGA',
                'IMACOEUR1F',
                'KARLY2G',
                'KELIJAH1G',
                'REDOMBRELLE',
                'COCO1F',
                'SUNNING3G',
                'ISANA1F',
                'DOECHELLE',
                'DORFLY',
                'BODYBATGA',
                'DEBRAFFI',
                'ZOLA3F',
                'CHATOUDOU',
                'ELEPHANTBAM',
                'FLUOCHOUETTE',
                'FLUOHIPPO',
                'FLUOLANGE',
                'POINTELPELUC',
                'PYJRAFFGA',
                'PYJRAFFI',
                'MATHILDA1F',
                'BBNOMADEFDC1',
                'SAB3F',
                'SEMOC3F',
                'SETTY3F',
                'SUZI3F',
                'RICHELIEU3G',
                'SANDRA2F',
                'SECOEUR1F',
                'SEFLEURI7S1G',
                'SEROCK1G',
                'SHERRY1F',
                'SUZON1G',
                'SONNIA7S2F',
                'TIYACKBODGA',
                'TIYACKPOLOGA',
                'HARRY1G',
                'BERAYARMPULL',
                'ARMEL2F',
                'ARIEL2F',
                'PAUL0G',
                'BISOU7S2F',
                'ZINZIN7S3G',
                'ZARMY2G',
                'ZICASSO1G',
                'ZIKSALOM1G',
                'ZIKS1G',
                'NED1G',
                'NICETTE1F',
                'NINO1G',
                'PLEMENCE0F',
                'SAPHIA3F',
                'BOUTON7S3F',
                'STEP7S3G',
                'BODYFUNGAJL',
                'BODYGRIZZLY',
                'KV574NSY7S3G',
                'KE410BAY7S3F',
                'KV574PCY7S3F',
                'SANDIE3F',
                'PYJZOUBOSLAY',
                'SABI0F',
                'SURPYJYACKGA',
                'MARYLAYCHO',
                'NAZACHO',
                'DEBORA3F',
                'MAEVA3F',
                'ROSAS3F',
                'SOLINE3F',
                'ONCESKIRT',
                'KRAB2G',
                'CHOPOLNOR',
                'SWED2FISHTEE',
                'MACQUEST2012',
                'HEARTHAT',
                'SANDYNEWLEG',
                'CITADYCOL',
                'BELAM2G',
                'BERENA2F',
                'BIPAMM2F',
                'BIPAM2F',
                'BUAREZ2F',
                'CITADIBLOUSE',
                'BOUTYANGELO',
                'BOUTYASSISE',
                'BOUTYCARNET',
                'BOUTYDEUX',
                'BOUTYTOUJO',
                'KINRALIVRE5',
                'BABYCOOKDUO',
                'BEMILDUNGA',
                'AVENTETIQUAT',
                'BABYBJORNBIB',
                'BOXCAMEL',
                'MATELALANG',
                'ROUNDBOOK',
                'SOCALOOP',
                'BBPANTBASIC',
                'GRIGRANDPA',
                'POISSONADHES',
                'STICKDEAUVIL',
                'POUSSONTRACK',
                'MOUK1EN',
                'MOUK1FRFR',
                'DROITLONG',
                'LAYVELDUNGA',
                'P_TEECOUGIRL',
                'P_LAGSUNJEAN',
                'P_SUNDUNGLAF',
                'P_BOYJEAN',
                'P_LAGTRICOGI',
                'P_BOYCHEMCOU',
                'P_BOYCHEMLON',
                'P_JERGILAF',
                'P_LEGGING',
                'BABYSPIRIT',
                'COOKBOOKBIS',
                'BEAISOTHERME',
                'VIPMETRO',
                'P_LOTCULOTTE',
                'P_LAGWINDUNG',
                'P_LAGWINPANT',
                'P_WINDUNGLAF',
                'P_WINJEANLAF',
                'CROILONGBOD',
                'VIPVOYAGETE',
                'C1LAGTRICOGI',
                'HABIPLUIE',
                'MACVOLO',
                'P_BOYPULL',
                'VIPROUTARETE',
                'AVENTSTERINW',
                'BEATRANSATNW',
                'BABYBJORNCAP',
                'BEVELVETSHOR',
                'CHOMULTIRAY',
                'NINESIX1G',
                'BIBSECONDNEW',
                'P_SOLFEGE',
                'P_KELMOC',
                'BEDOLLGIL',
                'BILVANA3F',
                'SACOULEUR1G',
                'AMELIE2F',
                'BUGASACTRANS',
                'BUGACAMELEON',
                'BICHEL1G',
                'GIGI3F',
                'SESPERFOS1G',
                'LINDA2F',
                'ENDBODGA',
                'LGENTILLFEE',
                'LHISTOIRE',
                'BISROMSHIRT',
                'LILIGIL',
                'LLIREMAMAMAN',
                'LPOUDREPROUT',
                'C25061037510',
                'BEPLILAYPOLO',
                'DIANEPULL',
                'WILSONPULL',
                'C4SUNCALAF',
                'C4LAGSUNPANT',
                'P_ETEFIPANT',
                'STICPANT',
                'LAPINSWEAT',
                'RAYSOUPREA',
                'SOCHDUNGA',
                'C4CHOLAF',
                'THANKSBURNOF',
                'BEXPLODEB',
                'JOLILUDROBE',
                'PYJGALUDO',
                'CRIBVERSA0G',
                'DIMULTI',
                'BESHORTDUNGA',
                'BOCALSKIRT',
                'PETIPRINCOMF',
                'BEAMULTPORNW',
                'PETIPRINCOMG',
                'POICHADRESS',
                'BETWILLPAD',
                'SHIRECHAPFI',
                'BEJOLICABPUL',
                'PETIPRINBODG',
                'P_BOYSOCLOT',
                'P_LAGSOCLOT',
                'THANKSDRESS',
                'PRINCECOL',
                'DRAGEBLOOM',
                'P_SOCLOTGIRL',
                'BIMOU2G',
                'MEDAR1G',
                'LUNETTMARS',
                'YHACKER2G',
                'BOIFLOBLOUSE',
                'BOIFLOPANTFI',
                'IZINTILLE1F',
                'SUNNY2G',
                'TIA2F',
                'ETOILLEBRILL',
                'BOIFLOCARDI',
                'ELIE3G',
                'C4SOBEAUTY',
                'SUNDAY3G',
                'JQUESTIONCUR',
                'LHISTOIRNOEL',
                'LMOTORIKIKI',
                'KETTOU2F',
                'SAMINI1G',
                'TRICOTPANT',
                'ILIES1G',
                'DESTRIPES1G',
                'SAMARIPOSA1F',
                'SONFOU1G',
                'YVOILE3F',
                'PLAYACHO',
                'LANGECOULEUR',
                'SONY3G',
                'SESRAYURES3G',
                'INESS2F',
                'ISOLEIL2F',
                'ZORION3G',
                'MACARCEAU',
                'C5LAGTRICOGI',
                'C5LAGWINPANT',
                'CAIREBERM',
                'CITRONBERMUD',
                'REALPARKA',
                'SAHARAPARKA',
                'PYJBOYRAYE',
                'BELILIPULL',
                'BRINDICOMBI',
                'CAROTOPROBE',
                'SAGIBON',
                'ANTONHAT',
                'ALANPANT',
                'JANMULTIPULL',
                'JANPULLCHO7',
                'AVAHAPPYPULL',
                'ANTOVELOUCAB',
                'C5BOYSOCLOT',
                'LLAPROMESSE',
                'LPETITPOUCET',
                'JUDEMAILBLOU',
                'C5LAGTEELON',
                'P_DROITLONG',
                'C5SOUPLAF',
                'OLGAJEAN',
                'BODYMOUSTA',
                'JUDFANCYJEAN',
                'ORGEATBLOU',
                'MAILTEE',
                'STEVEREGJEAN',
                'NUITCONTINE',
                'PAREOSHORT',
                'DORSHANSEL',
                'NUITHARMORAY',
                'JANERAITEE',
                'BECHEMAO',
                'BECHERAYCAR',
                'KAWAIPANT',
                'JANEKNOTEE',
                'PYJGOUTTE',
                'CITRONBLOUSE',
                'JUDESHIRT',
                'STEVRAISOUP',
                'AVARAYESOUP',
                'RINGTEE',
                'COLLEVESTGA',
                'SURPYJHARMOF',
                'SURPYJHARMOG',
                'HARMOCOMBGA',
                'NOUSKABON',
                'NOUSKAMOUF',
                'CONTPANTGA',
                'CINSETKATR3F',
                'HARMOBLOUSE',
                'P_IFOURREF',
                'HARMOTIFI',
                'HARMOBURNOUF',
                'MARINPANTGA',
                'CHOPYJAGHOST',
                'MAKIGIL',
                'MARINBONGA',
                'AVAPOMANTO',
                'COLBRAMIRA',
                'ELASTLEED',
                'FLEAPULL',
                'BEMIVELDUNGA',
                'BEPLIVELDUNG',
                'BESEVGIL',
                'ROUNDTEE',
                'GIRLYPARKA',
                'TREDOUPILOTE',
                'BODYEAR',
                'JANEHATETEE',
                'QUESTHERITAG',
                'JUDEBLUSHIRT',
                'VELDORSVIP',
                'CHEMIBERAY',
                'PYJLAYGADROL',
                'BODCOW',
                'P_ABABOU',
                'BEMULTICBLOU',
                'BELDENJUPE',
                'BETORSADGIL',
                'TRICOROBE',
                'ZEBRABON',
                'BECHECKCHEM',
                'FELTYSCARF',
                'PYJDEGUIPLUM',
                'TWISTJEAN',
                'KARIBOUMOUF',
                'STEVPOLPJGIL',
                'WAKSWEAT',
                'TROISETREZ3G',
                'KATCENDI3G',
                'ILLUCOMPIED',
                'HARMOPULLGA',
                'SHERRY3F',
                'BODYFAON',
                'ANTONZIPGIL',
                'AUCHAMPI0F',
                'DESETOILES3F',
                'DOFAON',
                'COLLEPANTGA',
                'HORTENCIA3F',
                'DILION1G',
                'BOFIRSTAR',
                'BRIEFAON',
                'HARMODUNGAGA',
                'PRINCESSE3F',
                'BODYBADGES',
                'BODYDOTS',
                'BODYFORET',
                'ZAGE1G',
                'ENZALEG',
                'MASKECHEM',
                'TICTOCBLOU',
                'YINAPANT',
                'RATONMOUFLES',
                'EDITO2G',
                'DUCOEUR2F',
                'BODYCLOCK',
                'ZOOM3G',
                'MOSSCOL',
                'COLLEBLOUSE',
                'COLNICER',
                'BLINKPULL',
                'PYJSAPIN',
                'ACAT0F',
                'ALION0G',
                'ANTONMODETEE',
                'RAMBOU2G',
                'TARAGIL',
                'BELIGNESOUP',
                'PEOPLEGILET',
                'RATONBON',
                'RATONSCARF',
                'ANTONRIGOTEE',
                'FANNIE3F',
                'FUNKY3F',
                'OCOULEUR2F',
                'DADOU2F',
                'BEFIBREPANT',
                'EDGARPULL',
                'HARMOGIFURFI',
                'HANDPANT',
                'LOULA2F',
                'SEFLEURI1G',
                'IMOTARD2G',
                'KARENICHEMGA',
                'SHOCKPANTFI',
                'SHOCKBLOUSE',
                'TARALAYCOL',
                'IDOLE1G',
                'INNSCOMBI',
                'NOUSKALAYCOL',
                'BELCOLOR2F',
                'WOCKYSWEAT',
                'KOMONO3G',
                'LQUIVOLEFOU',
                'MELTINGVANIT',
                'HARMOROBE',
                'MARELLE3F',
                'RIGAMOUS1G',
                'ROCHAT2F',
                'RENTAISE2G',
                'EDLAYCHO',
                'BOLAVOL1G',
                'SHOCKINSOCK',
                'ANTONCHAPKA',
                'BETRENDYJEAN',
                'BLOCKJEAN',
                'FLOLAYDERAP',
                'MIGNOCHODERA',
                'SERAFIN1G',
                'SEVERNIS1F',
                'PATTYGIL',
                'TICTOCPANT',
                'BBNOMADEFDC2',
                'TICTOCPULL',
                'TICTOCSOUP',
                'TOUTSYLEG',
                'AVABICHSOUP',
                'OLGAHEARTSOU',
                'SUZYBELT',
                'CIELDRESS',
                'CHOUCHOU1G',
                'ADIVARIAL3G',
                'HANSELGASOCK',
                'OLGABISGIL',
                'SUZYVELSKIRT',
                'OLGARIGOSOUP',
                'SUZYCHEEKTEE',
                'ANTONBELT',
                'PLAIDTOUDOUX',
                'STEVCHICPANT',
                'STEVECAGOULE',
                'STEVETEDGIL',
                'CELAGILGA',
                'CROKETLAYCHO',
                'LOOPPOLO',
                'LOOPSOUP',
                'LOOPBELT',
                'NOACLASSPANT',
                'AVABELTPANT',
                'ALPSCARF',
                'BETORSARPULL',
                'HANDSOUP',
                'BLOCKSOUPU',
                'FUNSKIRT',
                'AVASCARF',
                'BEMODEPOLO',
                'BELILYTEE',
                'OCTAVEHAT',
                'STEVPOLMITEN',
                'STEVPOLSCARF',
                'SUZYGLOVBICO',
                'SUZYHATBICOL',
                'PEIGNFUN',
                'AMORBLOUFI',
                'CIELROBE',
                'FLOCKJEAN',
                'RAINDRESS',
                'RAINSHIRT',
                'INDYGIL',
                'BESOONCOMBI',
                'INNSMITTEN',
                'RAINSOUP',
                'BETEEGA',
                'BARRYCHEM',
                'HADOCKDUNGA',
                'HADOCKJEANS',
                'BAINPASTIL',
                'PENEBON',
                'DICKYTEE',
                'OLGASHIRT',
                'CCADEAUDPAM',
                'BETEEBROUSSE',
                'KHALOTEE',
                'BERIBRAY',
                'OCTAHOODCHE',
                'C6BOYDEBA',
                'C6BOYTEECOU',
                'P_DROITCOURT',
                'FLOCKJACHAT',
                'VESTETRICOT',
                'PYJAFRICAN',
                'MISTERJOG',
                'MONSIEURPOLO',
                'BOMAGILFI',
                'BOMAPANTGA',
                'DALIASWEAT',
                'SUNALICE',
                'DOUGFUNJEAN',
                'C6LAGTEELON',
                'C6BASBLOO',
                'PONCHGLOVE',
                'C6DEBALAF',
                'C6SUNCALAF',
                'C6COUPVENT',
                'C6TEELONGIRL',
                'AMORDUNGAGA',
                'BECHAMBLOO',
                'OMBRELBOB',
                'HANSELSCARFI',
                'TEALAYJOPANT',
                'BECHAMBDUNGA',
                'GOLDENGROUTA',
                'DALIAWILDJEA',
                'JOYGIL',
                'OCTACERFTEE',
                'ALIAHROBE',
                'BESARAPANT',
                'CALINEPANT',
                'BAINLAYCHERR',
                'AMORGILGA',
                'BOMAJEANFI',
                'NOEUDCOMBI',
                'PYJPARAP',
                'GLOPSWEAT',
                'HARMONIFSAIN',
                'HARMONIGANON',
                'HARMONIGSAIN',
                'BIKIJUPMULTI',
                'C6PANTJOGG',
                'PEIGNCROA',
                'PEIGNROBOT',
                'SHORTYPOOL',
                'DOUGPULL',
                'JOHNRAIPULL',
                'C6BOYTEELON',
                'CALIPSOPANT',
                'SLIPPOOL',
                'DOUGRAIHOPUL',
                'PUMP12E1G',
                'FABASIBERMU',
                'SHORTYAPRIL',
                'MAYOKAISSA',
                'BECOLORCHEM',
                'JADEPLISKIRT',
                'BEGILBICOL',
                'COOKYPOLO',
                'LOOSECHEM',
                'GOLDBODYFI',
                'POTTERTEE',
                'BOMABLOUSE',
                'BOMATEEGA',
                'MIRAMITTEN',
                'FRESAPOINPUL',
                'DALIASLIMPAN',
                'SACMAXI',
                'JEDESLESLETT',
                'LLAGRENOUILL',
                'LLIVRETOTOCH',
                'LRIGOLELULUG',
                'VICKYCHO',
                'BODTECKEL',
                'C6LAGSOCLOT',
                'JAJILLPANTGA',
                'SERREHTJAN',
                'ROMYLEG',
                'TEALAYGIL',
                'GHANAPULL',
                'KEYSPULL',
                'BROCGIL',
                'JAJILLCHEMGA',
                'DRESSLAYMAXI',
                'EPUREMIPANT',
                'EPUREMISHOE',
                'CHEMIAUTRUCH',
                'PYJAETE',
                'BEAJOURGIL',
                'LISONJOG',
                'CHEMIMAGIC',
                'BIMOU6S2F',
                'PYJSAFARIGA',
                'PYJSTRICOURT',
                'SUNLAYHIPOC',
                'SETGIL',
                'KYMARIN6S3F',
                'FRESATEE',
                'JADETEE',
                'RABBITCAP',
                'KIFLEURI3F',
                'PASTIFOUL',
                'PASTILBANDO',
                'AEREBOB',
                'AERECHAUSSON',
                'JUNKOBERM',
                'BEPARROTEE',
                'BENEWPIQPOLO',
                'LUNELAYCHO',
                'PASTILLAYCHO',
                'EPUREFIBODY',
                'ADINDOR6S0F',
                'AARONCV6S2G',
                'AARONPS6S2G',
                'BTSCLAS6S1F',
                'DRAGON6S2G',
                'BELINPANT',
                'DILIGNES3G',
                'SNICE6S2G',
                'CHEMIPSYCHE',
                'BIMOU6S2G',
                'REDCOMBIZIP1',
                'TIDYBLOUSE',
                'OTENNIS3F',
                'OVOITURE2G',
                'CORRALJEAN',
                'COLACCIPANFI',
                'GHANATEE',
                'SOFIPANT',
                'AUNI0F',
                'JATRAVAUX2G',
                'BEZOELEG',
                'FOLKLOBOB',
                'ROCHERRY1F',
                'RIVOITURE3G',
                'MONOICAP',
                'NUITCHARME',
                'DOENVOL',
                'KA373WP6S3F',
                'DOURAYE1G',
                'DOUXCOEURS2F',
                'MEDUSESHORT',
                'ZOEROBE',
                'SACALICE',
                'JEANEGIL',
                'LOCCAPANT',
                'BOXCOLOURS2G',
                'MILOSHIRT',
                'BAVOIRLANGE',
                'CHARMEPANTFI',
                'BRIEFMEX',
                'MEROUBERMU',
                'BODYBIGDOTS',
                'GOUACHEDUNGA',
                'JANGOBOB',
                'MATERDUNGA',
                'JANGOSHORT',
                '3DCASQUETTE',
                'BODYRIGOL',
                'KE420LG6S3F',
                'KV574NR6S1G',
                'ENVUETEEGA',
                'MAXWESTEE',
                'CARTOONBOB',
                'CARTOONDEBA',
                'GOLDSOCK',
                'MAXRAYCHO',
                'REMUS6S1G',
                'SINGY3F',
                'IDOTS3F',
                'CALVIDEBA',
                'HAROLDSHORT',
                'BASTER3F',
                'CHIARABOB',
                'CHIARACOMBI',
                'CLEAROBE',
                'KYLIAROBE',
                'MIKACOMBI',
                'MIKAROBE',
                'PENTYROBE',
                'YAKABLOUSE',
                'FRESAMAILJUP',
                'LLFESTINNOEL',
                'MIMOPANT',
                'MIMOPLIROBE',
                'TZIGSHORT',
                'DIVOINE2F',
                'LILOOTEE',
                'DULIEGE3F',
                'CHARMESHORGA',
                'KOME2G',
                'KANI2G',
                'MIKALAYCHO',
                'SESPATCHS1F',
                'SOLEIL6S1F',
                'ZORINO6S2G',
                'BOVICHY0F',
                'DELFINA6S1F',
                'DELTAVEL1G',
                'MIMOGLACETEE',
                'IRONDS2F',
                'LOUPING2G',
                'MINUS1G',
                'BEAVEILLNOEL',
                'REDSACPLUME',
                'HONAN1G',
                'ALDOPANOTI',
                'BARRI1G',
                'MIMOGIL',
                'SPORTY6S0G',
                'LILIFLYDRESS',
                'BADMIBLOUFI',
                'BADMIBOBGA',
                'BADMISHORJUP',
                'BADMIROBE',
                'AZURDEB',
                'ZINCPANT',
                'BOBGABB',
                'ETE3F',
                'CEREMONISOCK',
                'EPUREFISOCK',
                'ASIABLOU',
                'CLAPYMANT',
                'MATOUROBE',
                'MILOBOB',
                'GLOBECASKET',
                'MILESBLOUSE',
                'ZINGADEB',
                'TZIGDEB',
                'BABYMIRACLE',
                'NINNA1F',
                'C7BOYPOLOLON',
                'C7BOYROULPUL',
                'ANNATOPARKA',
                'BANOA1F',
                'BIXOU0G',
                'GLOUBITAB',
                'LOYALPULL',
                'STATESTEE',
                'TOUTDOUMIXT',
                'C7BANDO',
                'BEBOXERSTAR',
                'LOBETEESTAR',
                'PYJGIRLBE',
                'BEFORDJEANS',
                'C7PANTJOGG',
                'LOUPIOPULL',
                'PARISCOMB',
                'MANUGASSISE',
                'COMEFIDOU',
                'LOTBEFISHORT',
                'C7LAGSOUP',
                'MANUFISCRAT',
                'MANUFIDEUX',
                'PYJAPARTY',
                'PYJGABOUH',
                'PYJPOIDS',
                'FIDOBASJOG',
                'FIDOVESTE',
                'HENDRIXPANT',
                'LESUDCOMBGA',
                'LESUDDEBAGA',
                'ANNABIRDSOUP',
                'ANNASTARSOUP',
                'ANNADUFFLE',
                'JAPANDUFFLE',
                'PYJFILAYRABB',
                'PYJGARDEN',
                'JAPRIBPANT',
                'BEFLASHBLOU',
                'CLAPYSCARBON',
                'LEAPARKA',
                'PETITEE',
                'BEFRILDUNGA',
                'DOBOYRAYBE',
                'COUKIDRESS',
                'COUKISKIRT',
                'FEERIEMANT',
                'FEERIESKIRT',
                'JILPANT',
                'C7LEGMOUSGIR',
                'BEJABBERCHEM',
                'TOUTPETIDORS',
                'ANAMULRAIPUL',
                'BLIZBODYGA',
                'BLIZPANTFI',
                'LOUPIOBONCHA',
                'AARONPS6S2F',
                'MEMORBODCOLF',
                'JAPREVSHIRT',
                'BEMOUTHTEE',
                'MANIJEANS',
                'MEMORYBORAY',
                'MEMORYCHEMGA',
                'FIDOTEE',
                'ASIALINEGIL',
                'BLIZTEEFI',
                'ANNARAIDRESS',
                'MOINEAUDRESS',
                'MATOUBON',
                'MEMORYPANTGA',
                'SHEEPCAGO',
                'ANNASKIRT',
                'PARKA2BY3',
                'FILLEPARKA',
                'PYJFIPOIRE',
                'BERAYLAYCHO',
                'CLAPYLAYCHO',
                'ASIATYPOTEE',
                'JAPRINOTEE',
                'CLAPYSOUP',
                'NUITMEMORY',
                'ICLAC2G',
                'METALTEE',
                'DUOFIPARKA',
                'NUAGECOMBI',
                'ASIALOUPSOUP',
                'BECOLORGIL',
                'BEMIAPULL',
                'GRETABON',
                'FEERIEHAT',
                'FEERIESCARF',
                'GRETAJOGG',
                'KATYSOUP',
                'MEMORYBLOUFI',
                'MEMORYROBE',
                'ANNACAROSHIR',
                'JUNEFISAINT',
                'JUNEFIVOYAG1',
                'JUNEGANGELO',
                'JUNEGAVOYAG1',
                'TRESSYCOL',
                'ANAMODRAYCOL',
                'ANNAFOXCHO',
                'ANNAHEARTCHO',
                'ANNARAYCHO',
                'ANNELAYCOL',
                'COOKICHO',
                'MEMORYPULGA',
                'SERRE08HT',
                'ANNAMASKTEE',
                'ANNATREGPANT',
                'KNOTDRESS',
                'JAPTRAFICHO',
                'LOUPIOLAYCHO',
                'JILSHORTDENI',
                'POULAYCHO',
                'VIDEOCHO',
                'BEFRASETEE',
                'BALLPULL',
                'BEANGILET',
                'ETECOMBIGA',
                'CHAISELAQSPE',
                'LOTCLIC07HT',
                'JAPCARTEE',
                'JAPCLOWNTEE',
                'MOINEAUBLOUF',
                'MOINEAUROB',
                'BEAVEILLUTIN',
                'CHAISELAQUE',
                'CITYLINK2NEW',
                'BEPANTVAS',
                'BERIBSAR',
                'BOOKROBE',
                'ORYSKIRT',
                'BETRICOSOUP',
                'BEVELOTEE',
                'JILSHIRT',
                'DIPOUPEE2F',
                'DIRAISE2G',
                'DOUCOEUR7S1F',
                'DYCARO3G',
                'AMBRESHORT',
                'CHOUCAHAT',
                'JAPHAMISPOLO',
                'PANTINPANT',
                'BEDUNGALOU',
                'PANTINBON',
                'BEPANTALOU',
                'CALIROBE',
                'BELOLAPANT',
                'JAPUNIHAT',
                'ETEBODYGA',
                'MOINEAUCHEM',
                'MOINEAUJOGA',
                'MOINEAUPANGA',
                'MOINEAUSHOGA',
                'ASIAFUSEETEE',
                'FEERIEBLOUSE',
                'MOONYGIL',
                'MOONYSOUP',
                'BBCAGOULE',
                'BODYNEPAP',
                'OAKTEE',
                'DRAGONCFC72F',
                'VARIALMID73G',
                'DEPOIS1F',
                'MANISOUP',
                'CHOPOIBOA',
                'ADRAGONFLY0F',
                'JILBLOUSE',
                'AUNI7S0F',
                'BEBARRYPANT',
                'BEBIMATCHEM',
                'ANARAYCOL',
                'GRETALAYCHO',
                'BICOEUR7S3F',
                'CHAPKALUDIK',
                'COPSLAYCHO',
                'TRANSVERSOCK',
                'ANNABELT',
                'ANNAMODGLOVE',
                'ASIAMODSHIRT',
                'LASCOGIL',
                'BAGGYPULL',
                'TRESSYSHIRT',
                'DORAFFGA',
                'BODYDOUDO',
                'CARBONLAYCHO',
                'LOYALPANT',
                'NASASWEAT',
                'TIYACKROBE',
                'TIDOUMIPANT',
                'TIBOUROBE',
                'TIYACKGILGA',
                'TIYACKPANTFI',
                'TRESSYSKIRT',
                'BELINE3F',
                'BELLINA3F',
                'BODYWOOD',
                'DOBATGA',
                'BODYCITY',
                'BRIEFRAFY',
                'FLUOGIRAFE',
                'POINTELDOU',
                'LEO3G',
                'SALLY0F',
                'SEFAN1F',
                'SEVERNI1F',
                'SOPHIE1F',
                'HOMER7S3G',
                'BEABAPESMALL',
                'BUGANIDANLEG',
                'CHOUCADRESS',
                'HOCKEY3G',
                'ANNABANDO',
                'IMOTARD7S2G',
                'IMANOL1G',
                'ZINTAGE3G',
                'NALA1F',
                'NALBERT1G',
                'NANCY1F',
                'NANETTE1F',
                'PHILIP0G',
                'ETEBERETFI',
                'KAYAC7S2G',
                'KE420RRY7S3G',
                'SURPYJYACKFI',
                'FEERIECOL',
                'ONCETEE',
                'ONCEPANT',
                'BELCOLOR7S2F',
                'LLILETAITFOI',
                'LREVEILLETOI',
                'CHORENE',
                'STOPPOLO',
                'STOPTEE',
                'HEARTGLOVES',
                'BEXA1F',
                'BRIAM1G',
                'BOUTYONDINE',
                'BOUTYROUTA',
                'BOUTYVANITY',
                'BOUTYVOYAG2',
                'LJESUISENCPR',
                'COLACCIPOLOG',
                'AVENTETITER',
                'FAGNEBIS',
                'LITLEBODYART',
                'PANTBASIC',
                'PYJFISO',
                'BOXSURFER',
                'COMBIHADOCK',
                'BELFILLE2F',
                'VIPSCRAT',
                'MOUK3FREXP',
                'CAHIERTEXT',
                'P_CHOLAF',
                'P_JEANGIRL',
                'BODYBERAYFI',
                'ADVENTHAT',
                'CHERITEE',
                'BULLESOUV',
                'MACRECARO',
                'CHANCEAGNEAU',
                'TWEETYPANTF',
                'MAYOMONSTRE',
                'BETEESLUB',
                'BEABAIGNOIRE',
                'P_FAUCON',
                'MACNACELLE',
                'BODYBEUNIFIL',
                'CHOPYJGALOVE',
                'BEAPTIPOMINN',
                'DIELEPHAN',
                'STONEBLOOM',
                'BEABAPESBIG',
                'SKIRTDENIM',
                'BRILLE3F',
                'ABOTTE1G',
                'BEGETROUS',
                'CAMUS2G',
                'YFLEURS2F',
                'ENDPANTGA',
                'JULESLIMPANT',
                'IRISGILBIS',
                'BETIGERTEE',
                'C4LAGTEELON',
                'PETIPRINDRES',
                'SHIREGIL',
                'THANKSBONGA',
                'BADOUNE',
                'BEMULRAYPULL',
                'BABYTRANSAT',
                'BECUTETEE',
                'LEILOUTEEFI',
                'MYATEE',
                'BEKDATEE',
                'GABYTEE',
                'JEANBELT',
                'C4BELFILLE',
                'BOUMPANT',
                'NONAPANT',
                'PEIGNMODEFI',
                'FREEBLOUSE',
                'THANKSCARDI',
                'SOFTOURSLN',
                'KETCHUPDUNGA',
                'BOIFLODRESS',
                'JITOILE2G',
                'PETIPRINPANG',
                'DIRUBANS3F',
                'MAENA1F',
                'LCOMPTDANSER',
                'LGRANDEBETEO',
                'LNICOLASNOEL',
                'LVRAIPERNOEL',
                'SALOMINE2F',
                'ISAO3G',
                'DESPOUPEES1F',
                'MOUSTACHO',
                'RABIA3F',
                'REINE3F',
                'ROMANO2G',
                'UTAHDRESS',
                'HASSANA2F',
                'SHANDICHEM',
                'SHANDISAROUF',
                'KETCHUPCAP',
                'BOLETOBELT',
                'C5TRICOGILAF',
                'CITRONDEBFI',
                'DRAGEDEBA',
                'BOXERLONDON',
                'C5BOYROULPUL',
                'GRENIECOMBGA',
                'P_BODYRAYFIL',
                'MAYATUNIDEB',
                'BESCRAPTEE',
                'C5SOUPGIRL',
                'DEBUTAB',
                'MAITRETAB',
                'MARINBODGA',
                'SOPHIEGIRAFE',
                'TIMCARO',
                'JUDCARGOPANT',
                'MONSTERTAB',
                'JUDERAIPJGIL',
                'JUDERIBGIL',
                'AVAMULTIPULL',
                'ANTONCABAN',
                'P_CLAUDLONG',
                'LAMISDEMALOU',
                'LPETITPRINCE',
                'C5SOCLOTGIRL',
                'STEVEJEAN',
                'BEBUSTEE',
                'BEFUNJEANS',
                'NIGHTSTRIPES',
                'JOLIEROBE',
                'MAYONUMBERFI',
                'MAYONUMBERGA',
                'MARINJOGFI',
                'SETBEFI',
                'BODYINUIT',
                'DOFICLASS',
                'DOGACLASS',
                'FRIENDSTEE',
                'BEDUNGAVAS',
                'COLLEMANTFI',
                'NUITSHOCK',
                'CONTGILFI',
                'CONTPANTFI',
                'HARMOVESTE',
                'MARINDUNGAGA',
                'MARINGILFI',
                'CHOPYFICOEUR',
                'HARMOCALE',
                'HARMOPARKA',
                'BEMONSTERTEE',
                'CHOUMIRA',
                'JANFRILLPULL',
                'HARMOBURNOG',
                'HARMOPILFI',
                'TYPODUNGA',
                'TYPOSHORT',
                'HARMODEBAGA',
                'DORSCONTINE',
                'PEAKSCARF',
                'COEURDENROBE',
                'NIRVABLOU',
                'BCBBG2F',
                'FLEADRESS',
                'CHOFILUDERAP',
                'TERRDANGE',
                'BEVELJOGTOP',
                'DINOCHAPKA',
                'BEADOSEUSENW',
                'BRIEFCLASS',
                'KARENIBODFI',
                'ILLUSHOES',
                'COLPOISSON',
                'BODYFICLASS',
                'BOCARREAUX3F',
                'DESPOMMES3F',
                'CAMETROBE',
                'RIFLOWERS2F',
                'YINAGIL',
                'DELTA1G',
                'HARMOPANTGA',
                'SHOCKPOLOGA',
                'HARMOGILFI',
                'PYJCAKE',
                'PYJLAYBOUTIS',
                'PEOPLETEE',
                'OJANE3F',
                'ILIADE1F',
                'ILIDE3F',
                'IMAGINE1G',
                'VICHOO3F',
                'BEKADOTEE',
                'HARMODUNFI',
                'COLLEGECOL',
                'SABOOTS1F',
                'TREDOUBODY',
                'ALPPANT',
                'INKAPANT',
                'NINOU1G',
                'JOLIFAON1F',
                'MANU3F',
                'WOCKYTEE',
                'HADOKLAYCHO',
                'DOXMAS',
                'FRIENDLAYCHO',
                'HANSELGILFI',
                'BARIDO1G',
                'BLINKSCARF',
                'BLINKSKIRT',
                'HOCHETLANGE',
                'SHOCKINCOL',
                'WALESPANT',
                'HADOCKGILET',
                'BLOCKPULL',
                'LEEDSKIRT',
                'HARMONISOCK',
                'SEVELCRO1G',
                'BEBRODPULL',
                'CHAPYPANT',
                'TOUTSYPANT',
                'TOUTSYSOUP',
                'STEVEREVHAT',
                'BEJACQGIL',
                'CIELGIL',
                'CELADONSOCK',
                'CELAMOUFLEGA',
                'GLOPUNITEE',
                'MOSSKIRT',
                'STEVECODETEE',
                'CATYMOUF',
                'STORMPOLO',
                'BEBABYPANT',
                'IPPLUIE1G',
                'HADOCKCHEM',
                'TULIPEROUTA',
                'PYJANGEL',
                'HANSELSHOEGA',
                'HANSEROBECT',
                'INKAPOLO',
                'INKATEE',
                'ANGLAYFICHO',
                'PYJCONFETTI',
                'ALPMITTENS',
                'BELOTI',
                'WOCKYPANT',
                'PENECOMBI',
                'BEVELRAY',
                'ZAELIROBE',
                'CARAVELPANT',
                'DALIAPULL',
                'CARAVELBON',
                'C6LAGDEBA',
                'VIPPYJVEL',
                'DENTYROBE',
                'BEAUTYPARKA',
                'PONCHGIL',
                'CELAPANTGA',
                'BOMABOOTGA',
                'HANSCOMBGA',
                'BOMAROBCT',
                'ALIAHBOB',
                'POUPYPULL',
                'LOOPMITTENS',
                'HARMONIFVOY2',
                'HARMONIGVOY2',
                'BESHORTDENIM',
                'BEMANOTEE',
                'CALIDUNGA',
                'GOLDBANDOFI',
                'GOLDSETFI',
                'FORINDUNGA',
                'CALIPSOBERMU',
                'BODTRUCHE',
                'SETPANTJOGG',
                'SACMERMAID',
                'LLIVREONRZO',
                'C6CHOLAF',
                'JAJILLGILGA',
                'COLARDECO',
                'JOHNSWEAT',
                'BRAGAHTJAN2',
                'C6SOCLOTGIRL',
                'JAJILLTEEGA',
                'EPUREFICOMB',
                'EPUREGILGA',
                'EPUREFIBLOOM',
                'EPUREMIGIL',
                'FORINGILET',
                'MAXSHIRT',
                'SACSUNSHARK',
                'FABIOPOLO',
                'SHORTYCOLOMB',
                'MERMAIDFOUL',
                'MOLIEPULL',
                'NOEUDBANDO',
                'PYJRAYARC',
                'CARAVELRABOD',
                'CARAVELBODFI',
                'REDBASEISOFI',
                'JUNIORBABY',
                'BODRAFSO',
                'BOPIRATE2G',
                'DORAFISO',
                'CASQUETCOU',
                'ELOYPANT',
                'GRAFFPANTJOG',
                'AERETEE',
                'AARONMT6S1G',
                'LKTRAIN6S3F',
                'LKTRAIN6S3G',
                'YUKAPARKA',
                'DOUGBELT',
                'KINRAPOUPI',
                'KINRAVETA',
                'CALIPSOBASJO',
                'GHANALEGG',
                'COLACCICHEMG',
                'COLACCIGILGA',
                'LOOSEBERMU',
                'JADEMAILPANT',
                'BALPOIS3F',
                'SATIN3F',
                'JODOBEC1F',
                'SONZIP2G',
                'STRIPES3G',
                'DRAPHIPOCAMP',
                'HARNIEBLOU',
                'ROCOEURS3F',
                'ROSEPOIS1F',
                'WEDDVEST',
                'BEPOLOPAINT',
                'LOTBAVOIRS',
                'MAXRAISHORT',
                'PYJARIGOLO',
                'BODYOPTICAL',
                'BEFRILLEG',
                'ENVUEROBE',
                'GATSBYFISOCK',
                'CARTOONSHORT',
                'MAXSIXDEB',
                'RAYCRABOB',
                'IBAIGNE6S2G',
                'CHIARASHORT',
                'ENVUEGASOCK',
                'MIMOBRODSKIR',
                'LMOTORDUESTF',
                'LILOOROBE',
                'GRACOMIRAGE',
                'KAPO6S1G',
                'KENOVA2G',
                'MEROUDEB',
                'SERSOURIS1G',
                'BADMINGASOCK',
                'ALDOPOLO',
                'IWAVES2G',
                'MINTROBE',
                'HARMORI2F',
                'ILAN1G',
                'BADMIPANTFI',
                'MIMOBLOUSE',
                'DELFIA1F',
                'LOOBATEE',
                'AZURAYDEB',
                'GOSBYCOMBI',
                'LESUDROBLOOM',
                'LILIFRAIDRES',
                'MINTPLIDRESS',
                'COMEGAVOYAG2',
                'PLAIDTRICOT',
                'TROTTERPOLO',
                'ALDOFISHTEE',
                'CUZCOBLOUSE',
                'ZINGADRESS',
                'POIGNEESBIB',
                'SNAPPYDELUXE',
                'DIFATACHSUCG',
                'HEDI6S1G',
                'MEROUTEE',
                'AZURHAT',
                'ASIAMODVEST',
                'VIDEOTEE',
                'STATESGIL',
                'BETRENCHPARK',
                'SONGJEANS',
                'SONGTEE',
                'MANUFIHOUSSE',
                'MANUGABIGSAC',
                'MANUGAONDINE',
                'LLEPIQUENIQU',
                'LOHLESMAINS',
                'MEMORYBONGA',
                'MOINEAUPOLOF',
                'PYJAEAT',
                'CLAPYMOUF',
                'BELOLADUNGA',
                'LILITPANT',
                'SEASJOGG',
                'SWEDRAIPANT',
                'ANAMULRAIGLO',
                'JILRAIHAT',
                'BEKAWAIDUNGA',
                'LOYALHAT',
                'FIDOSOUP',
                'MEMORYHATFI',
                'MOINEAUTEEGA',
                'MATOUSOUP',
                'JILJUPE',
                'JILPICTEE',
                'TAILCOL',
                'DODINOFORT',
                'COPSDUNGA',
                'NUAGESHOE',
                'ASIARAIPULL',
                'JAPRAIHAT',
                'BEPOULTEE',
                'ANIMALTAB',
                'GRETAGIL',
                'JUNEFIANGELO',
                'JUNEFIHOUSSE',
                'JUNEFIROUTA',
                'ANNABIRDCHO',
                'MEMORYCOMBFI',
                'ROBOCHO',
                'BEABABYCALL',
                'LOTCLIC07HT2',
                'LOTELAST07HT',
                'LOTPONY08HT',
                'MEMORYGAGIL',
                'TOURTEROBONG',
                'TOURTEROSHOE',
                'REDNACELLENW',
                'BEHATEE',
                'KATYSHORT',
                'CHOUCAGIL',
                'CITADISHORT',
                'BALLDUNGA',
                'BEMOLLEGJUPE',
                'JAPUNIGLOVES',
                'JYCROWN2G',
                'ASIAMODCHECH',
                'DRAGONCFC72G',
                'MINNIE7S1F',
                'ASCARGOT0G',
                'CARBONEPANT',
                'NASASHIRT',
                'TOURGASOCK',
                'ASIAFUTURTEE',
                'LOYALSCARF',
                'SONGLAYCHO',
                'OAKPULL',
                'BECHELLE1F',
                'SCOOLPOLO',
                'CHOUCASOUP',
                'TYACKCOL',
                'ANNAPEAKSOUP',
                'ANNEAUTOUDOU',
                'POINTELHOCHE',
                'MABEL0G',
                'MARIE1F',
                'BEPOUTNIKTEE',
                'ARIANE2F',
                'PAM0F',
                'MAYALAYCHO',
                'KERALACOL',
                'DELOUISE2F',
                'LLIVREFILLES',
                'LLIVREGARCON',
                'SWEDPOLGIL',
                'AVENTETEPAIS',
                'BELU1F',
                'BIZAM0G',
                'BOUTYMETRO',
                'BOUTYPRIM',
                'DOMIEN1G',
                'BEATASSREGUL',
                'BASICOTENF',
                'BASISOUP',
                'SACMINILABO',
                'SOFTOURS',
                'BRIEFETE',
                'CHANCELBUGA',
                'CORDBASIPANT',
                'VIPANGELO',
                'VIPDEUX',
                'FRESHORDENIM',
                'MOUK2FRFR',
                'MOUK3EN',
                'BRACTATOOKID',
                'GATSBYTEEGA',
                'PYJCOLOMBE',
                'SETONTRACK',
                'ALBERPANT',
                'SHORTYLINES',
                'CHIVYDEB',
                'BANYC',
                'BASCHOOLBUS',
                'CHOUGRELOT',
                'LILIGRAFFITI',
                'ABBRE',
                'LATOUREIFFEL',
                'TANGRAM',
                'BEAREHAUSSE',
                'COCOONBAG',
                'BABYACTIVBIO',
                'BEANENUPHANW',
                'BEACUILLEREN',
                'RUBERTY',
                'TABLICHOUET',
                'SEPEINTRE1G',
                'BUGABEECANOP',
                'BUGABOOBEE',
                'ANNIA2F',
                'SIBASIS1G',
                'BUGACAMADAPT',
                'BOBSWEAT',
                'LIVREODEURS',
                'ENDSOCK',
                'DPAMHOCHSOCK',
                'DPAMCOUVERTS',
                'C4GILFINGIRL',
                'C4TEELONGIRL',
                'JTANGRAMS',
                'HACKERPANT',
                'DIANBOYJEAN',
                'JEANSWEET',
                'BERAYESOUP',
                'LUNLIVRE',
                'ZANCHEM',
                'BENINOJEAN',
                'BIRDYSHIRT',
                'AMSTERPANT',
                'CARTEFIDBE',
                'CRIBVERSA0F',
                'C4BOYCHO',
                'SOCHPANT',
                'BAINLAYRAY',
                'SACMAYOFI',
                'IRISEGDEUX',
                'BEDOTSGIL',
                'C4LEGMOUSLAF',
                'MARINPARKA',
                'OPOIRO3G',
                'EDITOU3G',
                'BOIFLOTEEFI',
                'BELISSIMA2F',
                'KEVAN2G',
                'FREEDOMDRESS',
                'SALSAPANT',
                'JMONSIEURBOU',
                'IRIVETS3F',
                'ITALY3G',
                'NELLPULL',
                'MESSAGEPOLO',
                'SAMI1G',
                'COMEBLOUSE',
                'COMEDRESS',
                'UZESPYJ',
                'BEWORDTEE',
                'CRISTALBON',
                'CRISTALBOOT',
                'CITRONBOB',
                'C5BANDO',
                'CHAISELOOP',
                'BEMIMIDUNGA',
                'BEPOLOPIC',
                'BEABABYPOTE',
                'JABBERSOUP',
                'TEDDYDOUNE',
                'LJENEVEUXPAS',
                'LPETITESBETE',
                'PYJANOIR',
                'PYJAIGLE',
                'DORSSHOCK',
                'PYJGARAYEBE',
                'CONTCHEM',
                'CONTCOMBGA',
                'BELOUPANT',
                'RAMIPANT',
                'ROUNDGIL',
                'STARCOMBI',
                'AVAMODVEST',
                'KLOERAYROBE',
                'MAKIPANT',
                'TYPOSOUP',
                'JANECLOCKTEE',
                'JUDERAIPOLO',
                'PLAIDBOOTY',
                'ANTOSLIMJEAN',
                'BECARROBLOU',
                'TARAPANT',
                'RAMISWEAT',
                'CEPEPANT',
                'NEWBORNCHOG',
                'JUDELINEGIL',
                'CINSETKATR3G',
                'BABYCOOKBAG',
                'CLASSICRIB0G',
                'JANESHORT',
                'BEGONIA3F',
                'ANTOPRINBELT',
                'DUPINGOUIN1G',
                'COLLEPANGAKA',
                'RAYURE1G',
                'WOODIPANT',
                'DOUCOEUR1F',
                'BODYTERRIER',
                'BELSOURIS1F',
                'FREEZSOUP',
                'BESAROUPANT',
                'CAMETBLOU',
                'COLLEPOLOGA',
                'SHOCKDRESS',
                'TREDOUCOMBI',
                'TARALAYCHO',
                'ANEMONE1F',
                'SONOISEAU1F',
                'COMPTISOCK',
                'BISOU2F',
                'JLESENQUETES',
                'LBORISTROUIL',
                'AVARAYCHO',
                'CIELTEE',
                'ECHARPEFILUP',
                'BOBINE0G',
                'ETOILE2G',
                'STEVPULLREV',
                'CHOLO1G',
                'ADIAMALA2F',
                'ADTRAINCF2F',
                'BIBILAYCHO',
                'HANDLAYCHO',
                'MELTINGBOULE',
                'TULIPEHOCHET',
                'GLOPTEE',
                'CATYBON',
                'DOUBONNET',
                'PEIGNASTRIPE',
                'STEVTRICOGIL',
                'BEBOX',
                'PENEPANT',
                'BEMUSICOLTEE',
                'LAPINFILLE',
                'OCTAPOLGIL',
                'BAINLAYSIXTY',
                'COUVMIRACLE',
                'PONCHAT',
                'C6TRICOGILAF',
                'OMBRELCOMBI',
                'DOUGNORMJEAN',
                'BOMABONGA',
                'BOMADUNGAGAR',
                'FALLONDINE',
                'ZAELIPANT',
                'LOOPPULL',
                'HARMONIFANON',
                'HARMONIFPRIM',
                'HARMONIGANGE',
                'HARMONIGVOY1',
                'DOSCAPHANDR',
                'MAYOALICE',
                'POTTERPARKA',
                'BOMASHOESFI',
                'BOMATEEFI',
                'CREOLFOUL',
                'JADECHECKBLO',
                'SETTEE',
                'DALIALOVTEE',
                'BAINCREOLE',
                'JCAHIERGRIBO',
                'LAFRIQUEZIGO',
                'LMESANIMAUXN',
                'LSAUVEQUIPEU',
                'MAYOLITCHI',
                'PARAJADE',
                'JAJILLCHEMFI',
                'EPUREFISAROU',
                'EPUREFICALE',
                'EPURETEEGA',
                'DALIAJEANBLO',
                'MARIMIDEBGA',
                'BRIEFLUDIVA',
                'GRACOCOCOON',
                'PYJPOSSESSIF',
                'GOLDUNGAG',
                'YUKALAYCHO',
                'LILIJUPE',
                'JADEPANT',
                'ARDECOCHO',
                'BEGRAFIKTEE',
                'BEIMPERBUS',
                'FLAVIEGILFI',
                'AKWAHSH6S1G',
                'DESLIGNES1G',
                'MOLIECIRE',
                'PYJECLAIRS',
                'REDHANDIES',
                'MAYOBB',
                'SUNDAY6S3G',
                'KINRAPOUPA',
                'KINRAVETR',
                'CHARMEGILFI',
                'COLACCIBOBGA',
                'DOCARRO2G',
                'KEYSPANTACOU',
                'DORSCOMEGA',
                'DEBAMIGNON',
                'LOOSEDEB',
                'WEDDUNIBERMU',
                'DOUVICHY1F',
                'DULILAS1F',
                'IPLUPIRATE1G',
                'LOCCABANDO',
                'LOULAYGIL',
                'MATERTEE',
                'RAFFINECOL',
                'ENVUECOMBFI',
                'CALVISHORT',
                'HAROLDTEE',
                'LLEPERENOEL',
                'SIRENSKIRT',
                'MIKATEE',
                'PENTYSHORT',
                'ABYSJEAN',
                'BADMISHORTGA',
                'CHARMDUNGAGA',
                'CHARMECHEMGA',
                'COLACCIBLOOM',
                'SOISEAU6S1F',
                'SARALOU6S1F',
                'SUNNYO6S2G',
                'DANNY1G',
                'MINFLEURS1F',
                'MEROUSHORT',
                'ORIO3G',
                'IOURI6S2G',
                'COMEFIROUTA',
                'COMEGADEUX',
                'BOARDBUGGY2',
                'COMEFIMETRO',
                'COMEGAHOUSSE',
                'GATSBYBLOUFI',
                'CLAPYROBE',
                'MILOSHORT',
                'TROTTERBERMU',
                'CHICASHORT',
                'MARLINTEE',
                'FIRSTCLASP12',
                'CHICSTERILIS',
                'EVOLVAPLUS12',
                'HEDOUARD6S1G',
                'BEABIBOZTET1',
                'JILCAPE',
                'PARISBON',
                'MANUFIANGELO',
                'MANUFIONDINE',
                'MANUFIROUTAE',
                'MANUGAVOYA1E',
                'MANUFIASSISE',
                'MANUFISAINT',
                'MATOUGIL',
                'MATOUBLOU',
                'OAKPANT',
                'FIDOPANT',
                'BEFIRSTPOLO',
                'ANAMULRAIHAT',
                'LOLAROBE',
                'C7BOYCHO',
                'C7CHOLONLAF',
                'FEERIEMITTEN',
                'JUNEFIDEUX',
                'JUNEFIVOYAG2',
                'JUNEGAROUTA',
                'JUNEGAVOYAG2',
                'ANAPEAKCHO',
                'BALLAYCHO',
                'BEDOTSLAYCOL',
                'CHOUFEERIE',
                'CLICFEERIE',
                'COLFEERIE',
                'KATYCOMBI',
                'CHOUCASCARF',
                'CHOPINGUDERA',
                'PANTINGILET',
                'MOONYTEE',
                'BODOTS1F',
                'PARAFEERIE',
                'PARANASA',
                'ASIARAYCHO',
                'MULTILAYCHO',
                'TOURFISOCK',
                'VERSASL2112F',
                'ANOURS0G',
                'LHASAGIL',
                'CARBONESOUP',
                'TIDOUMICOMB',
                'TIDOUMIHAT',
                'TIDOUMIMOUFL',
                'ELASTIBRUNCH',
                'DORSTIDOU',
                'FLUOELEPHANT',
                'NUNADRAPSENA',
                'BEAPOCHEISO',
                'LLPETITNICO',
                'AVENTPPMID',
                'LASCOTEE',
                'STOPBUSTEE',
                'BOUTYAPIS',
                'BOUTYVOYAG1',
                'GRACOMIRAGTS',
                'LDANSUNARBRE',
                'LMONLIVREDES',
                'LAUTOCOLLANT',
                'ENFANCEMUSIC',
                'ESSENTIELSOC',
                'TELETHON2006',
                'EPSAINTNICO',
                'BEATETIVARI',
                'PANTVELOURS',
                'VIPAPIS',
                'VIPSIROP',
                'MOUK4EN',
                'BODYBERAYGA',
                'P_BOYCHOLON',
                'BECANPANT',
                'DOUDOUSKIP',
                'SHERPASSOUP',
                'BEABIBVERRE',
                'DECOBOISNOEL',
                'IFLORA1F',
                'COCOONADRAP',
                'BOULIERGRIS',
                'DIPARK',
                'CINKSETDE3G',
                'DILOURS',
                'DUFEUTRINE',
                'NIGHTDRESSGA',
                'BEFIVEL2F',
                'PYJAGANOEL',
                'CHATEAUFILLE',
                'TINYDRESS',
                'BOARDBUGGY',
                'SHIREDEBA',
                'THANKSBODGAL',
                'CUBIXPANT',
                'JULIAJEAN',
                'C4TRICOGILAF',
                'YANKEETEE',
                'ARKDUNGA',
                'GREASEJOG',
                'FLOJEANDRESS',
                'BECOEURSOUP',
                'POICHALEGG',
                'PETIPRINCHEM',
                'PATTIPANT',
                'DINETPANT',
                'LPHILOMENEHI',
                'FREEBODFI',
                'SEBASI1F',
                'KATIANE2F',
                'OTULIPES1F',
                'SOLEILCORD',
                'DRAGECARDI',
                'ILSE3F',
                'SHANDICHAPFI',
                'POICHAGASOCK',
                'TEECLUB',
                'CAIREBODEBA',
                'PLASET',
                'ANIMALIASHIR',
                'TOUTPTIDORS',
                'SILVERDOUNE',
                'COUTURETAB',
                'SACNUMBERGA',
                'JANELEG',
                'LCAMPINGAUX',
                'ANTONJEAN',
                'C5LAGSOCLOT',
                'ORGEATJOG',
                'MAILDRESS',
                'CONTDUNGAGA',
                'HARMOCOMBFI',
                'MARINBODFI',
                'COLBRALEED',
                'JANERAILEG',
                'CHOPYJFIPOIS',
                'JUDEBISHIRT',
                'PYJADOE',
                'HARMOSOUSGA',
                'HARMOHATFI',
                'HARMOHATGA',
                'BAMBYSKIRT',
                'BLINKSHORT',
                'SIXDETROIS3F',
                'DOCOCOTTE',
                'BRAETOILE',
                'DILACERAYE1G',
                'BODYBOIS',
                'TARABON',
                'TICTOCBANDO',
                'HARMONI2F',
                'MASKETEE',
                'DUPOMPIER1G',
                'PEPPADAPT',
                'REVE1F',
                'ALVINPANT',
                'DOUILLETMOUF',
                'VICHOO2F',
                'KATYA2F',
                'ENZAGIL',
                'DOGGYBON',
                'WOODITEE',
                'JOLCOEUR3F',
                'BOTARA1G',
                'ALVINLAYCHO',
                'SURPYJBEAR',
                'ANTONCATTEE',
                'OSNOW1F',
                'JIPRINCESS1F',
                'JIROULE1G',
                'YPOMME2G',
                'JOMONSTRE1G',
                'LCROCAIMEMAM',
                'LMONSIEURPE',
                'DELAFLEUR3F',
                'SUZYLICHO',
                'CHAPYBLOU',
                'TOUTSYBON',
                'CAMETGIL',
                'LITHOUSSE',
                'AVABANDONOEU',
                'BEJACARDTEE',
                'DADHAT',
                'DADMITTENS',
                'ALPHAT',
                'EDGARMOUFLES',
                'INNSCARF',
                'A06041144040',
                'ANTONMODEGIL',
                'INKAHAT',
                'MOABCHO',
                'GLOPPANT',
                'HANSELBODGA',
                'HANSELCHEM',
                'HANSELSALGA',
                'WAKGLOVES',
                'CARAVELGIL',
                'SUNRAYLAY',
                'PYJSCAPHANDR',
                'DALIABLOU',
                'VIKSHIRT',
                'CALINEBON',
                'BAINLAYFRESI',
                'POUPYPANT',
                'HARMONIFONDI',
                'BIKILAURA',
                'COOKYPANT',
                'MILOPARKA',
                'COMBIFLAM',
                'MIRASCARF',
                'JMINICHERCHE',
                'LJOUONSAVECL',
                'BERIBDOT',
                'COLACCIGILFI',
                'LISONSHORT',
                'LISONTEE',
                'FORINPOLO',
                'BRACOHTFEV',
                'BRACOHTJAN',
                'FRESAPARKA',
                'JADETRENCH',
                'BELETOILES1F',
                'AERESHORT',
                'BEGIRAFTEE',
                'BEPOLODYE',
                'SQUAREPANT',
                'BERIZGIL',
                'LOULAYBOB',
                'ZOEVESTE',
                'DORSCHARME',
                'KINRAPOUPK',
                'KINRAPOUPN',
                'CALITEE',
                'AFLOWER0F',
                'AHIBOUX0F',
                'OPOURSONLEAF',
                'HARNIEROBE',
                'NUITGOLD',
                'PYJFANFARE',
                'KE410GR6S2G',
                'DITORTUE1F',
                'PYJBONBONS',
                'CHARMEROBE',
                'GOUACHEPOLO',
                'GOUACHESHORT',
                'GOUACHETEE',
                'MONOITEE',
                'CLEABOB',
                'KAISSABLOU',
                'KYLIABOB',
                'COLACCIGASOC',
                'MIKAGIL',
                'MINABLOU',
                'MINAPANT',
                'BEABIBOZTET2',
                'BRUNCHO',
                'KOLIN6S2G',
                'MAXTRANSPANT',
                'SESBRIDES1F',
                'HOURI2G',
                'MIRED2F',
                'KOUCOEUR2F',
                'APAPILLON2F',
                'LESUDBERMUGA',
                'COMEGACARNET',
                'COMEGASAINT',
                'COMEFIVOYAG1',
                'COMEGAVOYAG1',
                'DEVOLANT2F',
                'GLOBEBERMU',
                'GRUNHAT',
                'MILESKIRT',
                'CUZCOROBE',
                'DIFATACHSUCF',
                'BEALOTCOUVER',
                'BARYTON1G',
                'CHARMEFISOCK',
                'MEMORYBURNOF',
                'COMEFIJOUET',
                'MANUGAHOUSSE',
                'MANUFIPRIM',
                'MANUGASAINT',
                'LLENPROMEN',
                'BESTORMDUNGA',
                'BASILAYBANDO',
                'BLIZPOLOGA',
                'JILRAISCARF',
                'PARKABOY',
                'JUNEFIAPIS',
                'JUNEFIONDINE',
                'JUNEFITOUJO',
                'JUNEFIVANITY',
                'JUNEGAPRIM',
                'JUNEGASCRAT',
                'SACLAYELAN',
                'SACLAYKOALA',
                'SACLAYVACHE',
                'COUKISCARF',
                'BEANTEE',
                'MOINEAUSHOEF',
                'TOURTEROCOMB',
                'OMBRELLEDPAM',
                'APING0G',
                'ANNAMODSCARF',
                'ROISEAU7S1F',
                'SCOOLCHEM',
                'TIYACKDUNGA',
                'FLUOCHAT',
                'BEABAPESMID',
                'BODYBIG',
                'SPORTY7S0G',
                'SURSOIREEGA',
                'WOOLIELAYCHO',
                'LIVREUNEMAMA',
                'LLANIMAUXLOU',
                'LPIEGESETDEF',
                'AVENTECOUTBB',
                'AVENSUCENUI2',
                'FLOLAYCHO',
                'BOUTYASTOR',
                'AVENSUCETNW2',
                'KINRALIVRE4',
                'GLOUBIHAT',
                'BABYCOOKBOOK',
                'BBMOUFL',
                'BRACELETMERE',
                'CARTEFID',
                'DUORAIN',
                'TSUNIFORMH',
                'GRIMUM',
                'MACOMBRELLE',
                'COOKPANT',
                'CAMELEONBUGA',
                'STICKSUPERHE',
                'VIPANONA',
                'MOUK2EN',
                'MOUK3FRFR',
                'REDNACELLE',
                'MONCAHIER',
                'BODYBERAYFIL',
                'YORKSCARF',
                'AVENTPESMID',
                'GREENBIBGT',
                'BEALUNETBAND',
                'BOITETRESOR',
                'SHERPARKASKI',
                'JACMITTEN',
                'MACTWINTECH',
                'TWEETYJUPE',
                'AVENTIRLAINW',
                'BEAGOUTBIBNW',
                'BEPATCHJEAN',
                'BALINO0G',
                'BUGASACTOILE',
                'BUGABEEADAPT',
                'EDGEHOGPANT',
                'C4BANDO',
                'LKAMOAGENCE',
                'TOUAREGPANT',
                'THANKSAROUFI',
                'BAINLAYMOD',
                'LEILOUCHEM',
                'LARMANDEVACH',
                'LPREMTOUDOU',
                'LPTICAMELEON',
                'HACKERPOLO',
                'VIPROUTA',
                'CINSETKAT3F',
                'POICHATEEGA',
                'CLOCHETPANT',
                'MYAJEAN',
                'CLOLAYBLOUSE',
                'AKWAHADI2G',
                'ESPIEGLOU3F',
                'REDECHARPE2',
                'BAWIL1G',
                'BESHIRTCARO',
                'DRAGEDRESS',
                'NEWIMPER',
                'SPASHORT',
                'DINETROBE',
                'A10111040715',
                'CITRONSHORTF',
                'ASSISELOOP',
                'ALVINGILET',
                'PARAVOLGA',
                'MAGIQUEFDM11',
                'JUDSLIMJEAN',
                'BECROCTEE',
                'CROCKETDUNGA',
                'CROCKETPANT',
                'KAWAIDUNGA',
                'CONTICOMBFI',
                'DRESSMARIN',
                'MARINCHEMISE',
                'NOUSKAJEAN',
                'CLICMIRA',
                'SUSIEBLOU',
                'YORKISHIRT',
                'AVASKIRT',
                'BRACOLFRAISE',
                'TROISETRO2F',
                'JUDEBLUPOLO',
                'HARMOPILGA',
                'HARMOCHEM',
                'KANA1F',
                'BABYBJORNLI2',
                'DIRAYUVERT1G',
                'MINIBOY0G',
                'SHORTYMATRIO',
                'SONNIA2F',
                'CAMETPANT',
                'HANDDUNGA',
                'ANTONSWEAT',
                'KEROUAK2G',
                'WAKPANT',
                'LBORISBISOUS',
                'LCROCAIMEPAP',
                'MELTINGAPIS',
                'HAPPYLAYCHO',
                'MAKILAYCOL',
                'ARTY2F',
                'MIASHOE',
                'CIELBONFI',
                'CELASCARFGA',
                'KARENIPANTFI',
                'STORMSHIRT',
                'BARRYLAYCHO',
                'NOAPANT',
                'OCTAMITTENS',
                'A06041144041',
                'ANGORFICHO',
                'BIDULEPANT',
                'BEBRI',
                'FROSTYSOUP',
                'MOABGIL',
                'PYJGIPSY',
                'NOATEE',
                'SUZYSHIRT',
                'JOYLAYCHO',
                'P_BEBEBANDO',
                'PIXELPANT',
                'GUIRBERLINGO',
                'DALIAJOGG',
                'VIPDORSVEL',
                'SACMAYOFLAM',
                'FABIOPULL',
                'DOUGWORKJEAN',
                'AMORPANTGA',
                'HANSELMOUFI',
                'MOLIEROBE',
                'ROMYROBE',
                'CALINEBLOU',
                'DOUGSLIMJEAN',
                'BOMAPANTFI',
                'HARMONIGSCRA',
                'MAXPULLRAYE',
                'BENEWBLOUSON',
                'FANCYFOULARD',
                'MILOPULL',
                'COMBIFOLKLO',
                'COLSOFI',
                'C6CHOGIRL',
                'BRASOFI',
                'LISONPOLO',
                'MOLIETEE',
                'OMBRELBLOU',
                'JADEPLIPARKA',
                'JADERAISKIRT',
                'CHERRYFICHU',
                'CARAVELSHOE',
                'JOHNCABAN',
                'LISONROBE',
                'SACAPYJCROCO',
                'YUKAGIL',
                'GRACOMOSAIC',
                'WEDDBERMU',
                'BEDOGGYSWEAT',
                'BOBIPLAGE',
                'JADEISABLOU',
                'POPSBELT',
                'FORINTEE',
                'KINRAPOUPR',
                'KINRAVETN',
                'GRAFFTEE',
                'ENVUEPANTGA',
                'JOPOPEYE1G',
                'NUITEPURE',
                'DOEXTRA',
                'PYJACHATON',
                'JEANECHAP',
                'ALDOPANT',
                'BODYROSACE',
                'ALDOBERMU',
                'MARLINBERMU',
                'BODYLIONGA',
                'KV574RN6S2G',
                'GOSBYTEE',
                'LPERENOELESL',
                'ARASGILET',
                'YAKAPANT',
                'MINTVOLDEB',
                'PYJANGLAIS',
                'KINO1G',
                'MESRONDS1F',
                'HOUILI2G',
                'HUMOUR3G',
                'KIDALOME2F',
                'LESUDBOBGA',
                'BADMIPOLOGA',
                'COMEGAROUTA',
                'COMEGASCRAT',
                'COMEGAMETRO',
                'ABYSKIRT',
                'KEYSHIRT',
                'MARLINCRABDE',
                'TETISTEPUP1',
                'AZURPANT',
                'BRICCE2G',
                'MANUGASTOR',
                'COMEFIHOCHET',
                'COMEGADOU',
                'MANUFIASTOR',
                'MANUFIVOYA1E',
                'MANUGASCRAT',
                'MANUFIAPIS',
                'MANUGANGELO',
                'LLEDURMETIER',
                'KINRALIVRE3',
                'NUAGEBON',
                'JUNEFISCRAT',
                'JUNEGADEUX',
                'MOINEAUDUNGA',
                'DRAGONCFI71F',
                'LOTCLIC08HT',
                'SERRE07HT',
                'ANNAMODHAT',
                'MANILAYCHO',
                'BOUTYHOUSSE',
                'LDESBRUITSDA',
                'LBOUCLEOR3OU',
                'LLECOLEMATER',
                'LLSHERLOCKYA',
                'LNOSAMISRIGO',
                'CARNETNOTE12',
                'AVENTETINE',
                'GRIDAD',
                'VIPONDINE',
                'JEU7FAMILLES',
                'SCOUBIDOUS',
                'ETUISOBB',
                'BACHATNOIR',
                'SUPERTEEG',
                'ADDICS',
                'MAFUNAOTH3F',
                'CHOCOLAT09',
                'REDECHARPE',
                'REDGROUPO',
                'PYJFIMULTIRA',
                'BABYORIGIBIO',
                'HULOTGIL',
                'EDGEHODUNGA',
                'BRAFANTA2',
                'KRISTELLE3F',
                'ANITA3F',
                'CHATEAUGAR',
                'KANOE3G',
                'PARADEPULL',
                'LGAREMAITRES',
                'LTOMTOMETNAN',
                'JEANFLEEGIL',
                'FREDJEANBER',
                'C4JERGILAF',
                'STRASSOUP',
                'DUMULTI',
                'BENEWHAT',
                'MAYOGALUDI',
                'PUMPVERSA1G',
                'SACSUNGA',
                'SUNLAYCAT',
                'LUNETTORTUE',
                'SUNLAYTROIS',
                'BUTTERFLAYBL',
                'BALTICPANT',
                'REMU1G',
                'SUNLAYGA',
                'FLOBLOUSE',
                'KARLITO1G',
                'JYHERO1G',
                'ZOSHORT',
                'MACORGANISER',
                'MACVOLOPLUIE',
                'ANTOBASIPULL',
                'ALANSOUP',
                'C5CARDIGIRL',
                'CHAISEGOEAT',
                'PARALAN',
                'BRIEFPOULE',
                'P_CLAUDCOURT',
                'DORSDOUCE',
                'ASICORRIDO1G',
                'TWISTPUL',
                'JANREVERBLOU',
                'MARINCOMBGA',
                'BECUTEPANT',
                'BECUTEPULL',
                'NOEUPOLSCARF',
                'AVAPOISLEG',
                'BRAPERLE',
                'KATREDIX2F',
                'BRAHIB',
                'COLLEROBE',
                'MANILLEROBE',
                'ABIRD0F',
                'OALAN2G',
                'DOGGYMOUF',
                'TREDOUMOUFLE',
                'LCOFFRETSACL',
                'LSURILEZERTE',
                'MAGIEMOBILE',
                'SEMOUTON1F',
                'MIAPULL',
                'CRIBDRAGON0G',
                'HIBPANT',
                'MINITAPEVEIL',
                'OCTAVESCARF',
                'DADTEE',
                'TOUTSYLAYCOL',
                'RAINPANT',
                'GARDIANPANT',
                'KHALOBOOTEES',
                'LAPINGA',
                'DRESSALICE',
                'LUDICPARKA',
                'GOLDENGHOUSS',
                'ZAELIBLOU',
                'FALLHOUSSE',
                'HARMONIFROUT',
                'HARMONIFSCRA',
                'HARMONIGAPIS',
                'HARMONIGROUT',
                'HARMONIGVANI',
                'BIKIALICE',
                'SUNSURFER',
                'CLICHTJAN',
                'DALIABELT',
                'DRAPSHARK',
                'IMPERBBFI',
                'BOXURBANS',
                'PYJTRAVAUX',
                'SHORTYDOTPOI',
                'BRAHTJAN2',
                'JOHNPARKA',
                'JOHNRAIPANT',
                'CALIELAYCHO',
                'PLUMTYLAYCHO',
                'YSKATE3G',
                'DESCERISES1F',
                'BRONXPANT',
                'JOTRAVAUX1G',
                'COMBIMAXI',
                'NUITJAJILL',
                'HARNIEGIL',
                'MATERCASKET',
                'SUNFIPOOL',
                'BADMICOMBIGA',
                'LUNCALINMIMI',
                'ARTYTEE',
                'SEPURPLE1F',
                'SERAFLEURS2F',
                'SESRIVETS1G',
                'A15091147308',
                'REDCHANCELFL',
                'MIMOSADEB',
                'COMEFIAPIS',
                'COMEGANGELO',
                'COMEGASIROP',
                'GATSBYCHAPFI',
                'MILOCLUBDEB',
                'BIBSTEPUP3',
                'HEIDDI6S1G',
                'BOP1G',
                'P_LAINBRASS',
                'JERSEYCOUV',
                'MANUFIVOYAE2',
                'MANUGADEUX',
                'MANUGAPIS',
                'NUNALITSENA',
                'JERSEYTROUSS',
                'MANUFIVANITY',
                'MANUGAVANITY',
                'ANAMULRAISCA',
                'JUNEFICARNET',
                'CUSTOCHAUF12',
                'BEYOKEPOLO',
                'SANDYCHEICH',
                'TIDOUMISHOES',
                'QUESTKIDSTON',
                'BRACOL07HT',
                'WILOUCHEM',
                'WILOUPANT',
                'LLESVACANCES',
                'TROUSSALLAIT',
                'BOUTYTROUSSE',
                'AVENTSTERIMO',
                'AVENTKITNVNE',
                'MONTREADI',
                'OMBRELLEBUGA',
                'GRIGRANDMA',
                'GRACORAINUNI',
                'LAYPICOTEE',
                'STICKPALMAS',
                'VIPSAINT',
                'VIPHOUSSE',
                'TRIPPCOUSSIN',
                'MACTRANSAT',
                'CORDCHEM',
                'COSMOSCARF',
                'SOFTOURSVEL',
                'RAVA2F',
                'ADIEUMES9ANS',
                'PUZZLE',
                'COMPORTEMENT',
                'MAMERE',
                'AVENTPESEVOL',
                'LOUISTRIPYTI',
                'LIVRERECETTE',
                'BUGGYBOARD',
                'KAFKA1F',
                'BUGAPORTGOB',
                'MESFLEURS1F',
                'BEAHOUSSE',
                'JORN3G',
                'CYBEXATON',
                'LPETITPIRAT',
                'LKESKECE',
                'JMEMOIREARC',
                'LOUESTCHARLI',
                'DAVECOTEGIL',
                'BOUMGIL',
                'RECREJOPANT',
                'VIPMOBILE',
                'JULIAPANT',
                'SACMAYOGA',
                'POICHABANDO',
                'PATTISHIRT',
                'CLOCHETCHO',
                'PEGMATELANAC',
                'LCHOSESDNOEL',
                'ACOWBOY0G',
                'BOGIRAFE2F',
                'IMBABABA1G',
                'REDTIKAMAK',
                'SERONDS1F',
                'SONETOILE1G',
                'SHANDISALAGA',
                'NELLJEAN',
                'NEWGIL',
                'JUDERAISCARF',
                'MACQUESTPLUI',
                'JUDERAIPULL',
                'DRAGEPANTGA',
                'PYJAKARO',
                'BEHOYTEE',
                'BEKLOEBLOU',
                'CROCKETPULL',
                'BRIEFBOOK',
                'JANMULTISCAR',
                'BODPOP',
                'SACNUMBERFI',
                'WHEREGILET',
                'FOXIEMOUF',
                'FILTOISE',
                'TWISTSKIRT',
                'SERRENOEUD',
                'BRAGACORDE',
                'MAGICTOUJO',
                'MATRIROBE',
                'LCLUBCHATONS',
                'LAPINLANGE',
                'ANTONYLCHAP',
                'HADOCKTEE',
                'TULIPEBOULE',
                'MELTINGHOCH',
                'LEEDSCARF',
                'HANSELBLOUSE',
                'BEROBOTEE',
                'BELODEBA',
                'SUNLAYFLAM',
                'SUNLAYMERMAI',
                'C6LAGTRICOGI',
                'DALIASLIMJEA',
                'BEHISSEOPULL',
                'GOLDENFROUTA',
                'FALLROUTA',
                'DOUGRAIGIL',
                'DOUGHOODPULL',
                'GOLDROB',
                'MIMOLEG',
                'BODYCRAB',
                'SETBERMU',
                'SETGILET',
                'JADECARDI',
                'AMORGILFI',
                'LCHARLIEPOCH',
                'LLECHATPITRE',
                'LMONDEVUDEME',
                'CLICHTJAN2',
                'JAJILLDUNGA',
                'BRACOBRUNCH',
                'SIXTIESFICHU',
                'PLUMTYGIL',
                'SUNLAYVAGUE',
                'TIDYPANT',
                'JADEBLOUSE',
                'BEAJOULAYCHO',
                'RUFFLEROBE',
                'OPARAPLUIE2F',
                'KINRALIVRE1',
                'DRAPMAXI',
                'TULLEGIL',
                'BODYLAOKA',
                'MIKRAYLAYCHO',
                'PENTYTEE',
                'MINASET',
                'KAISSALAYCHO',
                'MINTUNIDRESS',
                'COMEGATROUSS',
                'VOLIAMANT',
                'GLOBETEE',
                'TETISTEPUP4',
                'CHICVEILLEBB',
                'COMEGAHOCHET',
                'COMEGAJOUET',
                'MANUFIBIGSAC',
                'MANUGAROUTAE',
                'MANUGAVOYA2E',
                'LLEVIOLONYVO',
                'LMAXLILIVEUL',
                'LSURMAFLEUR',
                'LLEPRINTEMPS',
                'KINRACHAUFF',
                'DOBOYRAYBI',
                'COUKIMANT',
                'JUNEGACARNET',
                'JUNEGASAINT',
                'JUNEGATOUJO',
                'SACLAYECUR',
                'COUKIHAT',
                'BEAPACKPTIPO',
                'SACMETROPOLE',
                'CUSTOCHAUG12',
                'BRA08HT',
                'PACKACCBCOOK',
                'REDGLOBESAC',
                'MAXIPORTION',
                'LMINICHERCHE',
                'BOUTYBIGSAC',
                'BOUTYSAINT',
                'BOUTYSCRAT',
                'BABYCOOKSOLO');

	    //$listValidModelsOpenBar= array_merge($array1);

	    return in_array($model, $listValidModels,true);

	}

}//class
