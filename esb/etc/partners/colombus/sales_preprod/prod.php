...
abstract class ObserverSales extends EaiObject
{
	...
	protected function _transco(EaiEvent $event) //beforeSend(EaiEvent $event)
	{
        ...
        if (1 /*or (int)$data['grand_total'] > 0*/) {
			...
			if($rootIncId == 'A812951' or $rootIncId == 'A813042' or $rootIncId == 'A813056') {	////TODO bug a resoudre "Store credit amount cannot exceed order amount." sur le save	
				$obj->setElement(false);
                  return false;
			}
            ...
            */
            ...
            } else {    //TODO enlever cette condition quand on gerera les taxe par pays  (pour l'instant c'est UE = taxe FR, le reste sans taxe)
                $codeRegime = 'F';
            ...
            foreach ($data['items'] as $index => $orderItem) {
				...
				if (1 or !isset($trancoTauxStandardTVA[$countryFacturation])) {	//tant que tva que francaise ou pas de TVA...
